...
- **#11 複数ビューア対応 (実装完了)**: ビューアタブを「ユーザーが追加 / 削除 / リネームできる N 個のビューア」に拡張。spec は [docs/spec-multi-viewer.md](../docs/spec-multi-viewer.md)。
   ...
   - **TabContextMenu サブメニュー**: 「ビューアへ移動 ▶」を新規 `MoveToViewerSubmenu` 子コンポーネントとして実装。マウス hover 150ms 遅延で開閉、キーボード `→/Enter` で開く / `←/Esc` でサブから親へ戻る / `↑/↓` でラップアラウンド。画面右端で左に flip。`viewers.length === 1` のときは項目ごと非表示。`tab-context-menu-root` 共通 wrapper で click-outside 判定をスコープ。`setActiveViewer` は呼ばず src 側で作業継続 (VS Code "Move Editor to Other Group" と同等)。
...
- `frontend/src/features/viewer-grid/`: ビューアタブ (BSP ツリー / DnD / 画像表示)。`internal/imgread` に対応。`layout.ts` (純関数) は vitest でユニットテスト済み、`useDnD.ts` は pointer events 自前 + `elementFromPoint` でドロップ先を解決。
