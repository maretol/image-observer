...
- **一覧 (分類) タブ**: サイドカー (`_classification.json` 正本 + `.csv` 初回 import) ベース。タグチップ / 信頼度 / 検索 / サブディレクトリ別アコーディオン / プレビュー + 編集を 1 画面化した SampleModal (#93) / 競合検出 / 子→親マージ / 複数選択 + バルクオープン / Card 右クリックメニュー / fsnotify による silent auto-merge
...
- **フロントテスト**: `frontend/` で `npm run test` (vitest)。純関数ユニットテスト中心。auto-save キューの race は `useAutoSaveQueue` を renderHook で、baseline reset 判定は `sampleEditBaselineSync` を純関数テストで検証 (#110 B)。デフォルト env は node、renderHook など DOM が要るファイルのみ `// @vitest-environment happy-dom`
