...
- **#11 複数ビューア対応 (実装完了)**: ビューアタブを「ユーザーが追加 / 削除 / リネームできる N 個のビューア」に拡張。spec は [docs/spec-multi-viewer.md](../docs/spec-multi-viewer.md)。
   ...
   - **テスト**: Go state v6 マイグレーションテスト (v5 ロスレス昇格 / v5 invalid layout fallback / v6 round-trip / 空配列 / activeViewerId mismatch / 重複 ID / 名前 sanitize / 上限 8 切詰めの各ケース)、vitest viewers (layout / serialization / DnD 純関数群)、`go test ./...` / `npm run test` 全通過、`tsc --noEmit` クリア、`wails build` 通過。件数は最新の `go test ./...` / `npm --prefix frontend test -- --run` を参照 (AGENTS.md A-1)。
...
- **#19 フォルダー監視 Phase 1 (実装完了)**: 一覧タブの現在フォルダを fsnotify で監視し、外部からの画像追加 / 削除 / `_classification.json` 編集を **silent auto-merge** で反映。spec は [docs/spec-folder-watch.md](../docs/spec-folder-watch.md)。ビューア側自動 close / バナー UI / 監視対象の細かい設定は Phase 2。
   - **新規パッケージ `internal/watcher`** (`watcher.go` + `watcher_test.go`): `Manager` (`Start(root)` / `Stop()` / `Current()`) + `ChangedPayload` (`addedFiles` / `removedFiles` / `renamedFiles` / `sidecarChanged`) + `EmitFunc` + `DefaultDebounce` + `ClassificationChangedEvent` 定数。`EmitFunc` を DI することでテストでは payload をキャプチャ、本番では `app.go` で `runtime.EventsEmit(a.ctx, watcher.ClassificationChangedEvent, p)` に流す (Wails runtime API は ctx 第 1 引数を取るので NewApp で closure capture、Wails runtime 依存は `app.go` 側に隔離)。実装: 起動時に root を明示 `Add` (失敗で error 返却) → 配下のサブフォルダは best-effort で walk + Add (初期 Start 用の `addSubtree` ヘルパ; 戻り値 `bool` のみで画像 path 収集はしない、#75 8th)、Create イベントで新規 dir は `addSubtreeCollect` (Create 用、戻り値 `(bool, []string)` で見つけた画像 path slice を返す) で再帰 Add し、画像 path を `changedAccumulator.discoveredImagePaths` に登録して**後続の inotify Create を 1 回だけスキップ** (mv-in 中の concurrent writer による double-count race の解消)、200ms `DefaultDebounce` で `changedAccumulator` に集約 → 1 flush で 1 emit。Chmod / 隠しファイル (`.`始まり) / 非画像 Create は無視するが、**非画像 / 非 sidecar の Remove / Rename はディレクトリ削除 / 移動の可能性があるため `anyChange` フラグで flush トリガ + `removeSubtreeFromWatch(st, ev.Name)` で `ev.Name` 配下の watchedDirs + fsnotify watch を一括解除する** (inode-tracked watch が残ると rename 先 subtree の変更が現 root の event として流れ続け `current folder only` が崩れる; #75 15th)。画像ファイル分岐の Remove / Rename も同じく `removeSubtreeFromWatch` 呼び出し (画像拡張子付きディレクトリ `photos.jpg/` のような誤判定で watch が inode 残留するケースの defensive cleanup)。**画像ファイル Write は counter 不変・anyChange 不変だが debounce timer は延長** (大きい画像コピー中の Create → Write 連続で flush が早すぎてフロントが未完成ファイルを開く race を防ぐ; Write 単独は pending 空のままなので flush no-op で Phase 1 の「既存画像 content edit はサムネ更新しない」挙動も維持)。明示 Stop は pending を discard、Events 不期 close は warn + flush。loop の Errors ch は close 検知で nil 代入してタイトループ回避、**Errors からの err 受領時は `anyChange = true` を立てて debounce timer も再起動** (inotify queue overflow など取りこぼし通知でフロントが古い一覧を保持しないための recovery 経路; #75 7th)。dir Create 分岐は `os.Stat` ではなく **`os.Lstat`** で symlink を検出し、非画像 / dir symlink は addSubtree を呼ばず anyChange のみ立てる (外部 dir への symlink で watch tree を target まで広げてしまう問題、分類スキャナも symlink を辿らない設計と整合)、**画像拡張子の symlink は image-Create branch に fall through して addedFiles++** (分類スキャナは Lstat ベースで extension のみ判定するため symlink でも entries に含めるので、payload と re-Load 結果を揃える; #75 8th)。`addSubtree` は **初期 Start 用 (collect なし、戻り値 `bool` のみ) と Create event 用 (`addSubtreeCollect`, 戻り値 `(bool, []string)` で画像 path を dedup map に park) の 2 経路に分割** — 大きな画像フォルダの初期 Start で不要な POSIX path 文字列を確保→GC するスパイクを回避 (#75 8th)。
   ...
   - **フロント `useClassification.ts` 拡張**: `Opts.watchMode` を受け取り、ライフサイクルを **2 つの effect に分離** して Start/Stop の IPC 競合を避ける。(1) Folder-watch effect (Start のみ): `watchMode == null` (settings 未ロード) は何もしない / `(folderPath 空 || "off")` は `StopFolderWatch` / それ以外は `StartFolderWatch(folderPath)`。**cleanup で明示的に Stop しない** — Go 側 `Manager.Start` 自身が atomically 旧 watch を tear down するため、`Start("A") → Start("B")` シーケンスを保ち、`Stop+Start` 混在 (goroutine 順序で B が誤って Stop される race) を回避。(2) Event-subscription effect: マウント期間中 1 度だけ `EventsOn(CLASSIFICATION_CHANGED_EVENT)` を bind、`handlerRef` 経由で最新ハンドラを呼ぶことで state 変化で再 subscribe しない。**cleanup でも StopFolderWatch を呼ばない** — React.StrictMode の dev 二重マウントで cleanup → 再 setup 順に走ると、非同期 Stop が次の Start より後に Go 側に届いて dev だけ監視が止まる race が出るため。最終 Stop は `main.go` の `OnShutdown` → `app.shutdown` → `Manager.Stop` が拾うので goroutine リークも無し。EventsOn ハンドラは `editingRef` / `conflictRef` / `mergePromptOpenRef` (state mirror) を参照。`requestGenRef` で payload 世代番号を発行し、await LoadClassification 中に新しい payload が来たら古い結果は **成功経路と catch 経路の両方で** commit 前に破棄 (race fix)。defer 中の payload は `{ fresh, folder, capturedGen }` の 3 フィールド形で `pendingResultRef` に park し、replay 時は `performReplay` ヘルパに集約 (folder 照合 → 不一致破棄 / `capturedGen !== requestGenRef.current` → park 後に他経路が gen を bump したと判定して破棄 / mtime 照合 → ずれていれば `++requestGenRef` を発行して再 Load し、in-flight watcher Load との後着衝突 (rollback) を防ぐ。再 Load の catch / success どちらも世代チェック / folder チェック後に進める。失敗時は手動 reload と同じ setError + setLoadResult(null) + error toast / `decideAutoMerge` 再評価で commit-editing-removed / 再 defer / commit を選択)。`(mergePrompt.open, conflict)` と `editing.open` の 2 つの変化監視 effect が `performReplay` を起動する。auto-merge 中の LoadClassification 失敗 (success path 同様 catch path も世代 / folder 照合済) は `setError + setLoadResult(null) + error toast` で手動 reload 経路と揃える。**`requestGenRef` は watcher handler / replay reload / `loadInternal` (= 手動 reload / openFolder / auto-load on mount / conflict-resolve / merge-resolve / delete-conflict-retry) の全ての非同期 Load 経路に共通の世代カウンタ**として働き、どの 2 つの Load が交錯しても古い方の commit がスキップされて新しい結果が消されない。spinner (`loading`) の解除だけは `requestGenRef` ではなく **専用の `loadingTokenRef` を `loadInternal` だけが bump** する別トークンで判定 — watcher / replay が `requestGenRef` を進めても manual reload の finally が `setLoading(false)` を skip して spinner が取り残されるのを防ぐ (#75 6th レビュー対応)。`StartFolderWatch` は **catch 経路だけでなく success 経路にも stale check** を入れ、Wails IPC が call ごとに別 goroutine で dispatch されることで Start("A") → Start("B") が Go の `m.mu` 取得順で逆転して残ってしまうケースに対し、`folderRef.current` / `watchModeRef.current` を再評価して現在 intent (Stop / 別 folder の Start) を再 dispatch する。Start same-root は Go の `Manager.Start` で no-op short-circuit する設計、Stop は idempotent なので二重 IPC は副作用なし (#75 7th レビュー対応)。watcher handler / performReplay の **`LoadClassification` await 後** にも `watchModeRef.current === WATCH_MODE_OFF` 判定を入れる (entry 時点のチェックだけだと in-flight payload が off 切替後でも処理されてしまう。Round 8 で指摘された AGENTS.md H-8 race マトリクスの「mode post-await」列が抜けていた問題)。`loadInternal` は stale 時に **`null` を返す**ことで caller (`openFolder` / 初期 autoLoad) が postLoadFlow を superseded フォルダに対して走らせないようにし、`openFolder` は `setFolderPath` 直前に `folderRef.current = picked` を同期書きして「state 更新→effect 更新までの gap で旧 watcher event が新フォルダ扱いされる」窓を塞ぐ。`folderRef` / `watchModeRef` は render-time に同期 (ref mutation は React の純粋性ルール内)。handler 冒頭で `watchModeRef.current === WATCH_MODE_OFF` ガード (StopFolderWatch では配送済み event を取り消せないため off 切替直後の event を捨てる)、handler / performReplay の reload 成功経路で `setError(null)` を呼ぶ (前回失敗の error が成功後も残らないように)、`StartFolderWatch` の catch は `folderRef` / `watchModeRef` で stale 判定して旧 Start の失敗を現フォルダの toast に誤帰属しないようにする、`StopFolderWatch` も `.catch` で unhandled rejection を防止、watchMode → off 遷移時には `pendingResultRef` を破棄 (編集中に外部変更を defer → off に切替 → 編集 close で auto-merge が後から走るのを防ぐ。`performReplay` 冒頭でも watchMode off なら破棄)。**`dispatchWatchIntentRef.current()` の中で `StartFolderWatch(folder).then()` の success 経路から `silentRecheckAfterStart(folder)` を呼ぶ** (intent reconcile で folder/watchMode が一致するときのみ) — 初期 LoadClassification と watch 確立の間に追加されたファイルを no-toast / no-spinner で吸収する経路。entriesEquivalent 一致なら何もせず、差分があれば既存の `decideAutoMerge` (defer / commit-editing-removed / commit) ロジックに乗せる (#75 9th)。**`inFlightDeletesRef: Map<folder, Set<filename>>` を導入し、`deleteOne` の冒頭で `(folder, filename)` を add → finally で必ず delete**。handler / `silentRecheckAfterStart` の entriesEquivalent 判定では `payload.folder` (handler) / 引数 `folder` (recheck) で Map 引きしてから **cur (= stale な現在表示) 側だけ in-flight filename を strip** して fresh と比較するため、sidecar save の完了タイミングに依存せず DeleteImage の self-echo を確実に抑制できる (fresh 側は strip しない — 削除 IPC 中に同名ファイルが外部で再作成された場合は fresh.entries に通常通り surface するため、#75 25th thread D で asymmetric strip に修正; 旧実装は両側 strip で再作成差分を隠してしまっていた)。**Map key を folder にしている**のは、フォルダ切替後に旧 folder で進行中だった delete の filename が新 folder の watcher self-echo 判定に混入して同名ファイルの差分を消す race を避けるため (#75 9th 初版 = Set<string>、#75 21st thread A で folder-scoped Map に変更)。**ローカル mutation (`saveEdit` / `deleteOne`) の成功時に `++requestGenRef.current` を実行**し、watcher / replay / silent recheck / manual reload の in-flight Load を stale 化することでユーザー編集が後着の Load に巻き戻されないようにする (#75 10th)。**`silentRecheckAfterStart` は generation を `snapshot only` で参加** (bump せず読み取りのみ) — 他経路を巻き戻さないが、他経路に supersede されたら自身は drop。bump も含めると initial-load (openFolder の loadInternal) を stale 化して postLoadFlow がスキップされる (#75 11th 修正で snapshot 化)。`Start` / `Stop` IPC は `dispatchWatchIntentRef` の単一エントリポイントに集約され、**completion (then / catch どちらも) で folderRef / watchModeRef を再評価して必要なら再 dispatch する fixed-point パターン**で Wails IPC の dispatch 順逆転に収束 (Start same-root no-op + Stop idempotent なので二重 IPC は副作用なし; #75 10th)。監視 root vanish 時は loop 抜けに加えて `st.watcher.Close()` で fsnotify の fd / reader goroutine を即時解放 (#75 10th, thread B)。**`pendingResultRef` に `capturedGen` を持たせ、`performReplay` 冒頭で `requestGenRef.current` と異なれば他経路 commit があったと判定して pending 破棄** (mtime 比較だけでは entries だけ変化のケースで巻き戻し可能; #75 11th)。**フロント `reload()` で `dispatchWatchIntentRef.current()` も呼ぶ** ことで、root vanish 後 zombie 化した Manager.state を手動 reload で復旧 (Go 側 zombie 判定 + 再構築は既存; #75 11th)。**deleteOne の競合リトライで `loadInternal` が null (失敗 or supersede) を返した場合は早期 return** で local patch + gen bump をスキップし、新しい Load / 次の watcher event に reconciliation を任せる (#75 11th)。**`postLoadFlow` は各 await 後 (`PreviewChildSidecars` / `confirm` / `CreateEmptyClassification`) に `folderRef.current === path` を確認**し、folder switch 中に旧フォルダの merge prompt / sidecar-create confirm が新フォルダ画面で surface しないよう gate する (#75 12th preemptive)。**`resolveConflictForce` / `resolveMergeMerge` / `resolveMergeSkip` も mutation IPC 成功直後に `++requestGenRef.current` を bump** — Round 10 で saveEdit / deleteOne にだけ入れた gen bump rule を AGENTS.md H-7 波及 grep の観点で全 mutation 経路に展開 (#75 12th preemptive)。**`editingRef` / `conflictRef` / `mergePromptOpenRef` も `folderRef` / `watchModeRef` 同様に render-time sync** — useEffect 同期だと open 直後 effect 発火前に届いた watcher event が「閉じている」判定で即 commit + mtime 取り込み → 後の saveEdit が競合検出をすり抜ける問題があった (#75 12th thread A)。**`silentRecheckAfterStart` は `initialLoadInFlightRef` (loadInternal が awaiting 中かどうか) を check し、true なら setTimeout で defer** — snapshot only でも同一 gen の older snapshot 初回 Load の後着 commit に上書きされる race を回避 (#75 12th thread C)。**watcher.go の dir Create 分岐は `acc.discoveredImagePaths` を共有 dedup set として活用**し、parent walk と child Create event が同じ画像を見つけた場合の double-count を防ぐ (#75 12th thread B)。**`loadResult` を null にする 3 catch 経路 (loadInternal / handleWatcherPayload / performReplay reload) では `resetEntriesDependentState` ヘルパで editing / conflict / mergePrompt / pendingResultRef / selected / selectAnchor も一括 clear** — entries を失った状態でこれら残置 state が残ると EditPopover の内部 state が "open" のまま defer 判定が誤動作、同名ファイル復活で popover 再表示する問題があった (#75 13th)。**`openFolder` (folder 切替) でも同ヘルパを呼ぶ** (selected だけクリアでは editing / conflict / mergePrompt が旧 folder context のまま残る; #75 14th)。**`loadResultRef` も render-time sync** (handler self-echo 判定で setLoadResult 直後の watcher event が古い entries を見る race を回避; #75 14th)。**全 mutation 経路 (saveEdit / deleteOne / resolveConflictForce / resolveMergeMerge / resolveMergeSkip / postLoadFlow の最終 loadInternal) で disk IPC 完了後の state commit 前に `folderRef.current === cur` check** — folder switch 中に旧 folder の mutation 結果で新 folder state を上書きする問題 (gen bump では解決しない、#75 14th)。**watcher.go の Remove/Rename 分岐は `watchState.watchedDirs` 共有 set で dir-vs-file 検出** — image-extension dir (e.g. `photos.jpg/`) の削除は `removedFiles` に計上せず `anyChange` のみに倒す (`w.Remove` 戻り値は IN_IGNORED 非同期処理でタイミング依存になり信頼できない、#75 14th)。`acc.removedPaths` で IN_DELETE + IN_DELETE_SELF + IN_IGNORED 多重発火を per-window dedup。**Remove/Rename 時は `removeSubtreeFromWatch(st, ev.Name)` で `ev.Name` 配下の watchedDirs + fsnotify watch を一括解除** (inode-tracked watch が残ると rename 先の subtree 変更が現 root の event として流れ続ける `current folder only` 違反、#75 15th)。**root の Remove/Rename event は hidden filter (`isHiddenName(base)`) から除外** (root のフォルダ名が `.` 始まりだと root vanish event が捨てられ loop が dead fd で hang、#75 15th)。**Go `repository.SaveJSON` は expectedMtime > 0 かつ sidecar 不在を ErrConflict として返す** (旧コードはコメントだけ「conflict 扱い」と書きつつ fall-through で silently 再作成していた; 編集中に外部から sidecar 削除されても次の saveEdit が conflict 検出を通って resolveConflict prompt が出る、#75 16th)。**`silentRecheckAfterStart` の success 経路でも `setError(null)` を呼ぶ** (handler / replay reload 経路と同じく、直前の Load 失敗の error が silent recheck で復旧しても残らないように、#75 16th)。
   - **設定 UI**: `ListSection` に「フォルダ自動監視」segment (自動 / オフ) を追加。hint は debounce 数値を含まない曖昧な表現 ("短い遅延の後に反映") に統一して D-1 重複を回避。segment の `value` および `useClassification` の off 判定は `frontend/src/features/settings/watchMode.ts` の `WATCH_MODE_AUTO` / `WATCH_MODE_OFF` 共通定数から import (Go 側 `internal/settings.WatchModeAuto` / `WatchModeOff` と TS 側 `watchMode.test.ts` の D-1 同値テストで両側 pin)。
   ...
   - **degraded mode**: `StartFolderWatch` 失敗時は warn トースト + log で済ませ、手動 reload で動作継続。再 Start の試行はフォルダ切替に限らず、**手動 reload も `dispatchWatchIntentRef.current()` を呼ぶため再 Start を試行する** (#75 11th。フォルダ切替を経ずに復旧したい場合の経路)。watcher goroutine が backend channel close で先に死んだ zombie state は、次の `Start(同 root)` で `goroutineExited` 判定により再構築される。`mtime` ベース conflict 検出 / merge prompt は既存経路をそのまま流用。
   - **テスト**: `internal/watcher` の Go integration test (Linux inotify 実機) と vitest (`watcherPolicy.test.ts`) を追加。Go ↔ TS 同値テストは 2 系統あり、(1) event 名 (`TestClassificationChangedEventName` ↔ vitest `CLASSIFICATION_CHANGED_EVENT === "classification:changed"`)、(2) WatchMode リテラル (`internal/settings.TestWatchModeValues` ↔ `frontend/src/features/settings/watchMode.test.ts`) を両方 pin。どちらかの定数を rename すれば両側のテストが落ちる (AGENTS.md D-1)。件数は最新の `go test ./internal/watcher/...` / `go test ./internal/settings/...` / `npm --prefix frontend test -- --run` を参照 (AGENTS.md A-1)。`go test ./...` / `tsc --noEmit` / `wails build` 全通過。
- **#58 Card 右クリックメニュー拡張 Phase 1 (実装完了)**: 一覧 Card の右クリックメニューを **単一 / バルクの 2 モード** に拡張。spec は [docs/spec-card-context-menu.md](../docs/spec-card-context-menu.md)。設定 UI (項目順 / 表示制御) / バルク削除 / フォルダ移動は Phase 2 へ後ろ倒し。
   ...
   - **テスト**: `features/classification/cardContextMenuLogic.test.ts` を新設 (`computeCardContextMenuMode` と `canBulkSplitOpen` 両純関数の各ケース)。ファイル名は `CardContextMenu.tsx` との大文字小文字違いだけの衝突を避けるため `Logic` suffix (Copilot レビュー #58 thread #8)。state schema / settings schema / IPC は変更なし。`go test ./...` / `npm --prefix frontend test -- --run` / `tsc --noEmit` / `go vet ./...` 全通過。件数は最新の `npm --prefix frontend test -- --run` を参照 (AGENTS.md A-1)。
...
# Go テスト (root の app_test.go も含む。main は go:embed all:frontend/dist を持つため
# 事前に `wails build` を一度通すか、CI 同様に `mkdir -p frontend/dist && touch frontend/dist/.ci-placeholder`
# でダミーを置いておく必要がある)
go test ./...
...
│   │   └── state_test.go    # v6 round-trip + v5 → v6 migration + ratio clamp + activeId fallback + duplicate-id reject 等 (件数は最新の go test ./internal/state/... を参照、AGENTS.md A-1)
...
- **`internal/watcher`** (#19): フォルダ単一監視 + debounce/coalesce。エクスポート: `Manager` / `NewManager(emit EmitFunc) *Manager` / `NewManagerWithDebounce(emit, d)` (テスト用) / `Start(root) error` / `Stop() error` / `Current() string`、型 `ChangedPayload` (`Folder` / `AddedFiles` / `RemovedFiles` / `RenamedFiles` / `SidecarChanged`) / `EmitFunc func(ChangedPayload)`、定数 `DefaultDebounce` (200ms) / `ClassificationChangedEvent` ("classification:changed")。依存: `github.com/gofsnotify/fsnotify`、`internal/classification` (`SidecarJSON` 文字列定数のみ — `Service` 等は呼ばない)、`internal/imgfile` (画像判定)、`internal/logging`。Wails runtime は import せず、`app.go` で `EmitFunc` に `runtime.EventsEmit` を注入する形を採る (テスト容易性 + 依存方向のシンプル保持)。`Start` は再入可能 (旧 watcher を Stop してから新 root に切替、root の Add 失敗は error 返却 / 子の Add 失敗は warn して続行)、Start 同 root でも goroutine 死亡 (`done` ch 閉鎖) を検知すれば zombie state を再構築。`Stop` は idempotent で pending を discard。OS によらず初期 Start は `addSubtree` helper (戻り値 `bool` のみ) で全サブフォルダを個別 Add (`AddRecursive` 不採用、OS 差を消すため)、Create イベントでは `addSubtreeCollect` (戻り値 `(bool, []string)` で画像 path slice を返し double-count dedup map に登録) で再帰 Add (mv-in / cp -r の取りこぼし回避)。両者は内部 `addSubtreeImpl` を共通化。loop は Events !ok 時に `stopRequested` フラグで explicit Stop と unexpected backend close を区別 (前者は silent return、後者は warn + flush)、Errors close は nil 代入でタイトループ回避、Errors からの err 受領時は `anyChange = true` を立てて debounce timer 再起動 (queue overflow recovery)。dir Create 分岐は `os.Lstat` で symlink を検出し、**非画像 / ディレクトリ symlink は follow せず anyChange のみ、画像拡張子の symlink は image-Create branch に fall through して `addedFiles++` する** (分類スキャナが symlink でも extension で entries に含めるため payload と整合させる)。監視 root 自身が Remove / Rename された場合は loop が pending を flush → `st.watcher.Close()` で fsnotify の fd / reader goroutine を即時解放 → `return`、`goroutineExited` 判定により次回 `Manager.Start` で新規 watcher を構築する (= dangling watch を放置しない経路)。
