...
パッケージ間依存: `state` / `imgdecode` 依存なし (imgdecode は x/image のみ)、`imgfile` → `logging` (非 Windows ビルドのみ、Trash fallback の warn log)、`thumb` → `imgdecode` + `imgfile`、`imgread` / `classification` → `imgfile`、`imghash` → `imgdecode` + `imgfile` + `logging`、`watcher` → `classification` + `imghash` (定数のみ) + `imgfile` + `logging`、`winplacement` → `state` (型) + `logging` (windows ビルドのみ)、`winrestart` → `logging` (windows ビルドのみ)。循環参照を作らない。
