...
最終更新: 2026-05-10 (Phase 5: ビューア フレックスレイアウト)。state schema v4。viewer-grid を BSP ツリーに置き換え、タブの DnD で自由分割を実装。`internal/state` の `GridState` を `LayoutState` に置換、`internal/tree` は v1.2 で削除済み、`internal/classification` 既存。`frontend/` vitest 90 ケース (layout 55 追加)、Go テスト全通過 (state 14 / classification 等)、`tsc --noEmit` クリア、`wails build` 通過。**Phase 5 変更点**: (1) viewer-grid を `Grid { rows, cols }` から `Layout { root: SplitNode | LeafNode, activeId }` の BSP ツリーに変更。(2) ツールバー (`+ 行`/`+ 列`) 廃止、タブの **DnD** でレイアウト編集 (パネルの 4 辺に edge drop で分割、中央 / タブバーに center drop で移動 / 並び替え)。(3) 右クリックメニューは「閉じる / 右に分割 / 下に分割」の 3 項目に縮小、「別パネルへ移動」廃止。(4) `MAX_PANELS = 16` 上限。(5) Splitter 最小 100px + ratio 下限 0.05。残作業は `wails dev` での目視テスト + Phase G/H/I/J。
