...
- **#19 フォルダー監視 Phase 1 (実装完了)**: 一覧タブの現在フォルダを fsnotify で監視し、外部からの画像追加 / 削除 / `_classification.json` 編集を **silent auto-merge** で反映。spec は [docs/spec-folder-watch.md](../docs/spec-folder-watch.md)。ビューア側自動 close / バナー UI / 監視対象の細かい設定は Phase 2。
   - **新規パッケージ `internal/watcher`** (`watcher.go` + `watcher_test.go`): `Manager` (`Start(root)` / `Stop()` / `Current()`) + `ChangedPayload` (`addedFiles` / `removedFiles` / `renamedFiles` / `sidecarChanged`)。`EmitFunc` を DI することでテストでは payload をキャプチャ、本番では `runtime.EventsEmit("classification:changed", p)` に流す。実装: 起動時に `filepath.WalkDir` で全サブフォルダを `gofsnotify/fsnotify` に Add (OS 差を消す方針、`AddRecursive` は不採用)、Create イベントで新規 dir を incremental Add、200ms `DefaultDebounce` で `changedAccumulator` に集約 → 1 flush で 1 emit。Chmod / 隠しファイル (`.` 始まり) / 非画像 / 非 sidecar は除外。
   - **`gofsnotify/fsnotify` v0.0.6 を依存追加** (issue #19 コメント指定)。原 `fsnotify/fsnotify` ではなく active fork。Add に `Op` マスクを取る点だけ API 差。
   - **app.go バインディング**: `StartFolderWatch(folderPath)` / `StopFolderWatch()`。NewApp で Manager を構築し、emit コールバックは `a.ctx` を lazily 参照 (起動前に flush しても no-op)。`shutdown(ctx)` を新設し main.go の `OnShutdown` で呼ぶ (inotify FD / goroutine リーク防止)。`classificationChangedEvent` 定数で Wails イベント名を `useClassification.ts` 側と一致させる (D-1: 文字列重複は spec §16 + 双方の docstring で cross-reference)。
   - **`internal/settings.WatchMode = "auto" | "off"`** を additive 追加 (schema v1 のまま、per-field fallback で旧 settings.json から無痛 upgrade)。`DefaultSettings()` は `auto`。`Validate` / `applyFieldDefaults` / Go テスト 5 ケースに watchMode 経路を追加。
   - **フロント `features/classification/watcherPolicy.ts`** (純関数): `ChangedPayload` 型 (Go 側 mirror) + `formatChangeSummary(payload): string | null` (トースト文言) + `decideAutoMerge(ctx): { kind: "commit" | "commit-editing-removed" | "defer" }` (mergePrompt / conflict open → defer / 編集中 + 編集対象が削除 → exception / それ以外 → 即時 commit)。vitest 11 ケース。
   - **フロント `useClassification.ts` 拡張**: `Opts.watchMode` を受け取り、ライフサイクルを **2 つの effect に分離** して Start/Stop の IPC 競合を避ける。(1) Folder-watch effect (Start のみ): `(folderPath, watchMode)` 依存。watchMode === "off" / folderPath 空なら `StopFolderWatch`、それ以外なら `StartFolderWatch(folderPath)`。**cleanup で明示的に Stop しない** — Go 側 `Manager.Start` 自身が atomically 旧 watch を tear down するため、`Start("A") → Start("B")` シーケンスを保ち、`Stop+Start` 混在 (goroutine 順序で B が誤って Stop される race) を回避。(2) Event-subscription effect: マウント期間中 1 度だけ `EventsOn("classification:changed")` を bind、`handlerRef` 経由で最新ハンドラを呼ぶことで state 変化で再 subscribe しない。unmount cleanup で `unsub() + StopFolderWatch()`。EventsOn ハンドラは `editingRef` / `conflictRef` / `mergePromptOpenRef` (state mirror) を参照。defer 中の payload は `pendingResultRef` に park し、`(mergePrompt.open, conflict)` 変化監視 effect が両方閉じた瞬間に commit + decideAutoMerge 再評価 (editing-removed exception を replay 時も発火)。
   - **設定 UI**: `ListSection` に「フォルダ自動監視」segment (自動 / オフ) を追加。hint で「外部で画像が追加 / 削除されたり _classification.json が書き換えられると、一覧に約 200ms 遅延で反映」を明記。
   ...
   - **degraded mode**: `StartFolderWatch` 失敗時は warn トースト + log で済ませ、手動 reload で動作継続。次回フォルダ切替で再 Start を試行。`mtime` ベース conflict 検出 / merge prompt は既存経路をそのまま流用。
   - **テスト**: Go (`internal/watcher`) 10 ケース (single create / burst coalesce / 非画像 / 隠し dir / subdir + 配下 / sidecar write / Stop / 切替 / 同 root 二重 Start / chmod のみ無視)、vitest (`watcherPolicy.test.ts`) 11 ケース。`go test ./...` / `npm --prefix frontend test -- --run` / `tsc --noEmit` / `wails build` 全通過。
...
- **`internal/watcher`** (#19): フォルダ単一監視 + debounce/coalesce。エクスポート: `Manager` / `NewManager(emit EmitFunc) *Manager` / `NewManagerWithDebounce(emit, d)` (テスト用) / `Start(root) error` / `Stop() error` / `Current() string`、型 `ChangedPayload` (`Folder` / `AddedFiles` / `RemovedFiles` / `RenamedFiles` / `SidecarChanged`) / `EmitFunc func(ChangedPayload)`、定数 `DefaultDebounce` (200ms)。依存: `github.com/gofsnotify/fsnotify`、`internal/classification` (SidecarJSON 文字列のみ)、`internal/imgfile` (画像判定)、`internal/logging`。`Start` は再入可能 (旧 watcher を Stop してから新 root に切替)、`Stop` は idempotent。OS によらず `filepath.WalkDir` で全サブフォルダを個別 Add (`AddRecursive` 不採用、OS 差を消すため)、Create 時に incremental Add で新規 dir に追従。emit コールバックは DI で差し替え可能 (本番: `runtime.EventsEmit("classification:changed", p)`、テスト: 内部 channel に push)。
