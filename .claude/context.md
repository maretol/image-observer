...
- **#19 フォルダー監視 Phase 1 (実装完了)**: 一覧タブの現在フォルダを fsnotify で監視し、外部からの画像追加 / 削除 / `_classification.json` 編集を **silent auto-merge** で反映。spec は [docs/spec-folder-watch.md](../docs/spec-folder-watch.md)。ビューア側自動 close / バナー UI / 監視対象の細かい設定は Phase 2。
   ...
   - **フロント `useClassification.ts` 拡張**: `Opts.watchMode` を受け取り、`(folderPath, watchMode)` 依存の effect で `StartFolderWatch` + `EventsOn("classification:changed")` をペア wire、cleanup で `unsub() + StopFolderWatch()`。await Start 中の cancellation (`cancelled` フラグ) で reverse Stop。EventsOn コールバックは `editingRef` / `conflictRef` / `mergePromptOpenRef` (state mirror) を参照することで listener identity を `(folderPath, watchMode, toast)` のみに依存させる (毎キーストロークで再 subscribe しない)。defer 中の payload は `pendingResultRef` に park し、`(mergePrompt.open, conflict)` 変化監視 effect が両方閉じた瞬間に commit + decideAutoMerge 再評価 (editing-removed exception を replay 時も発火)。
