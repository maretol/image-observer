...
パッケージ間依存: `state` 依存なし、`imgfile` → `logging` (非 Windows ビルドのみ、Trash fallback の warn log)、`thumb` / `imgread` / `classification` → `imgfile`、`watcher` → `classification` + `imgfile` + `logging`、`winplacement` → `state` (型) + `logging` (windows ビルドのみ)。循環参照を作らない。
