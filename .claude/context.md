...
- **一覧 (分類) タブ**: サイドカー (`_classification.json` 正本 + `.csv` 初回 import) ベース。タグチップ / 信頼度 / 検索 / サブディレクトリ別アコーディオン / プレビュー + 編集を 1 画面化した SampleModal (#93) / 競合検出 / 子→親マージ / 複数選択 + バルクオープン / Card 右クリックメニュー / fsnotify による silent auto-merge / キーボード操作 (カードを矢印キーで 2D 移動 + プレビューで `t`/`c`/`n` で各編集欄にフォーカス, #115)
