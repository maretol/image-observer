...
- **`internal/state`**: セッション状態永続化 (schema **v6**)。export: `StateData`, `ViewerState`, `LayoutState`, `LayoutNodeState`, `TabState`, `ListTabState`, `ListFilterState`, `WindowState`, `Load`, `Save`, `SaveWindow`, `DefaultData`, `StateSchemaVersion`。`Load()` は v6 strict / v5 ロスレス昇格 (`migration_v5.go`) / それ以外は default fallback。`validateState` は viewer 数 / 名前 sanitize / activeViewerId 解決 / 重複 ID 拒否を実施。`SaveWindow` は Load→window 差し替え→Save で window フィールドのみ更新 (#129 の Win32 capture 用)
