...
- **フロントテスト**: `frontend/` で `npm run test` (vitest)。純関数ユニットテスト中心 + hook の race を renderHook で検証 (`useAutoSaveQueue` / `sampleEditBaselineSync`、#110 B)。デフォルト env は node、DOM が要るファイルのみ `// @vitest-environment happy-dom`
