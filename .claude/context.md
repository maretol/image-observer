...
- **一覧 (分類) タブ**: サイドカー (`_classification.json` 正本 + `.csv` 初回 import) ベース。タグチップ (「未分類」チップで未付与のみ絞り込み, #116) / 信頼度 / 検索 / 並び順ソート (手動 = sidecar 配列順 / ファイル名 / 更新日時 × 昇降、表示派生のみで sidecar は不変) + 並べ替えモード (手動ソート時のトグルで入り Card DnD で sidecar 配列順を書き換え。モード中はプレビュー / 選択 / タブ切替等を封鎖, #144) / サブディレクトリ別アコーディオン / プレビュー + 編集を 1 画面化した SampleModal (#93) / 競合検出 / 子→親マージ / 複数選択 + バルクオープン / Card 右クリックメニュー (開く / クリップボードコピー #127 / 削除) / fsnotify による silent auto-merge / キーボード操作 (カードを矢印キーで 2D 移動 + プレビューで `t`/`c`/`n` で各編集欄にフォーカス, #115) / ダブり警告 (知覚的ハッシュ dHash + しきい値判定、Card ⚠ バッジ + 確認モーダル + ペア単位 dismiss を `_duplicates.json` に永続化, #136)
- **ビューア (複数対応)**: 複数ビューア (リネーム + クロス DnD) × BSP ツリー (任意分割、上限 16 パネル) × タブ (DnD 並び替え / 分割発生 / 閉じる / 右クリックメニュー) × 画像 (ズーム / パン / ResizeObserver 追従 / 右クリック → クリップボードコピー #127) × キーバインド (`Ctrl+W` / `Ctrl+Tab` / `Ctrl+0` / `Ctrl+1` / `Ctrl+±` / `Ctrl+Shift+1..9`)
- **設定 UI**: ログ / 一覧 (multi-select mode) / ビューア (画像上限 / wheel mode / タブ最大数 #148) / サムネイル (size / mode / worker) / タグ色 / 外観 (UI scale) / フォルダ自動監視 / ショートカット一覧
...
- **ウィンドウ位置復元**: 終了時の位置/サイズ/最大化を次回起動で復元。Windows は Win32 WINDOWPLACEMENT (#129, `internal/winplacement`、マルチモニタ位置不具合を解消)、非 Windows は Wails runtime + ポーリング (#86)
- **自動再起動登録**: Windows にアプリを Restartable App 登録 (`RegisterApplicationRestart`, #129/#86 のセッション+ウィンドウ復元と合わせて crash / hang / Update 再起動後に元の状態へ復帰)。Windows のみ (`internal/winrestart`, #133)、非 Windows は no-op
- **タスクバー連携**: Windows タスクバーのサムネイルプレビューに「前/次のビューア」ボタン (`ITaskbarList3` サムネイルツールバー + window subclass, #149, `internal/wintaskbar`)。クリックは EventsEmit → FE (`useTaskbarViewerSwitch`) がビューアタブを巡回。非 Windows は no-op
- **CI**: ubuntu-latest 上で `go test` + vitest + `tsc --noEmit` + `GOOS=windows go build` (windows build-tag ファイルのクロスコンパイル check, #149) を pull_request トリガで実行 (`wails build` は CI で呼ばない)。Windows ビルド + portable/NSIS 配布は `v*` tag 押下時の `release.yml`
...
- ダブり判定ハッシュ (#136): `cache/duphash/<algo>/<2>/<30>.json` のフォルダ単位 JSON index (`sha256(フォルダ絶対 path)` キー、mtime+size 無効化)。dismiss はキャッシュでなくフォルダ直下 `_duplicates.json` (spec-duplicate-detection.md)
...
│   ├── imgdecode/                           # 画像デコード共有層 (thumb / imghash が使用)
│   ├── thumb/                               # サムネ生成 + ディスクキャッシュ + worker pool
│   ├── imgread/                             # 原寸読み出し + 寸法取得
│   ├── imghash/                             # ダブり検出 (dHash + キャッシュ + dismiss, #136)
...
│   ├── winhwnd/                             # main window HWND 自力取得 (Windows, #129→#149 で共有化)
│   ├── winplacement/                        # ウィンドウ位置復元 (Windows: Win32 WINDOWPLACEMENT, #129)
│   ├── winrestart/                          # Restartable App 登録 (Windows: RegisterApplicationRestart, #133)
│   └── wintaskbar/                          # タスクバーサムネイルツールバー (Windows: ITaskbarList3, #149)
├── frontend/
...
- **`internal/imgdecode`** (#136): 画像デコード共有層 (thumb の decode.go を移設)。export: `Decode`。アニメ形式は最初のフレームに潰す。AVIF は非対応 (spec-avif-support.md の WebView 委譲方針)
- **`internal/thumb`**: サムネ生成 + ディスクキャッシュ + worker pool。export: `Result`, `Config`, `Get`, `InitWorkerPool`, `CurrentWorkerCount`。`imgfile.IsImage` / `imgdecode.Decode` に依存
- **`internal/imghash`** (#136): ダブり検出 (docs/spec-duplicate-detection.md)。export: `Service`, `NewService`, `DuplicatePair`, `DuplicateReport`, `DHash`, `Distance`, `AlgoDHash`, `DuplicatesJSON`, `TempDuplicatesJSON`。dHash 64bit + フォルダ単位 algo 別キャッシュ index (`cache/duphash/`) + dismiss 永続化 (フォルダ直下 `_duplicates.json`、ハッシュ値ペアキー = rename 耐性)。folder 単位 mutex で Check / Dismiss を直列化。しきい値と mode は caller (app.go) が settings から解決して渡す。pHash + 切替設定は Phase 2 (別 PR)
...
- **`internal/state`**: セッション状態永続化 (schema **v6**)。export: `StateData`, `ViewerState`, `LayoutState`, `LayoutNodeState`, `TabState`, `ListTabState`, `ListFilterState`, `WindowState`, `WindowPositionUnset`, `Load`, `Save`, `SaveWindow`, `DefaultData`, `StateSchemaVersion`, `SortManual`/`SortNameAsc`/`SortNameDesc`/`SortMtimeAsc`/`SortMtimeDesc` (#144, `ListTabState.Sort` の許容値、無バンプ additive)。`Load()` は v6 strict / v5 ロスレス昇格 (`migration_v5.go`) / それ以外は default fallback。`validateState` は viewer 数 / 名前 sanitize / activeViewerId 解決 / 重複 ID 拒否を実施。`SaveWindow` は Load→window 差し替え→Save で window フィールドのみ更新 (#129 の Win32 capture 用)
- **`internal/settings`**: ユーザー設定永続化 (state とは別ファイル、`<UserConfigDir>/image-observer/settings.json`)。export: `SettingsData`, `Load`, `Save`, `Validate`, `DefaultSettings`, `SettingsSchemaVersion`, `MaxViewersHardCap` (#148, `state.maxViewersHard` との同値を state 側テストで pin) + 列挙定数 (`MultiSelectCheckbox` / `MultiSelectModifier` / `MultiSelectBoth` / `WheelModeZoom` / `WheelModeShiftZoom` / `ThumbnailModeLetterbox` / `ThumbnailModeCrop` / `WatchModeAuto` / `WatchModeOff`)。`defaultTagColors` シードは非公開 (参照型 export 回避、AGENTS.md B-1)。per-field fallback で破損値だけリセット
...
- **`internal/watcher`** (#19): フォルダ単一監視 + debounce/coalesce。export: `Manager`, `NewManager`, `NewManagerWithDebounce`, `ChangedPayload`, `EmitFunc`, `DefaultDebounce` (200ms), `ClassificationChangedEvent` (`"classification:changed"`)。依存: `github.com/gofsnotify/fsnotify`, `internal/classification` (SidecarJSON 文字列定数のみ), `internal/imghash` (DuplicatesJSON / TempDuplicatesJSON 定数のみ — dismiss 書き込みの self-echo ループ防止 ignore, #136), `internal/imgfile`, `internal/logging`。Wails runtime は import せず `EmitFunc` 経由で `app.go` から `runtime.EventsEmit` を注入 (テスト容易性 + 依存方向のシンプル保持)
- **`internal/winhwnd`** (#149): main window HWND の自力取得 (`EnumWindows` + 自プロセス PID マッチ、Wails 非公開のため)。export: `FindMainWindow` (windows ビルドのみ実体、doc.go は build-tag なしで非 Windows の `go build ./...` を通すための空 package)。#129 で winplacement に実装したものを winplacement / wintaskbar 共用に抽出
- **`internal/wintaskbar`** (#149): タスクバーのサムネイルツールバー (docs/spec-taskbar-viewer-switch.md)。export: `Setup`, `ViewerSwitchEvent` (`"taskbar:viewer-switch"`), `DirectionPrev`/`DirectionNext`。windows ビルドは main window を subclass (`SetWindowLongPtrW`、旧 wndproc は swap 前に atomic 公開) して `TaskbarButtonCreated`/`WM_COMMAND` を捕捉、`ITaskbarList3` (COM vtable 直叩き) で ◀▶ 2 ボタンを登録 (go:embed の ico を ICONDIR 自前走査)。HWND 探索は window 表示 (navigationCompleted) 待ちの retry 付き。COM 操作は wndproc 内 = UI スレッド限定、module state はプロセス生存中リセット経路なしの一方向。クリックは `Setup(onSwitch)` のコールバック (main.go が EventsEmit に写す)。非 windows は `ok=false` の no-op
- **`internal/winplacement`** (#129): ウィンドウ位置復元 (Win32 `GetWindowPlacement`/`SetWindowPlacement`)。export: `Restore`, `Capture`, `ToWindowState`, `FromWindowState`。windows ビルドは `winhwnd.FindMainWindow` で HWND を取得 → syscall。非 windows は `ok=false` の no-op で caller (`main.go`) を #86 経路へフォールバックさせる。`rcNormalPosition`↔`WindowState` 変換の純関数 (`winplacement.go`) は build-tag なしで Linux CI テスト。`main.go` の `OnStartup` (Restore) / `OnBeforeClose` (Capture→`state.SaveWindow`) から呼ぶ
- **`internal/winrestart`** (#133): Restartable App 登録 (Win32 `RegisterApplicationRestart`)。export: `Register`。windows ビルドは `kernel32!RegisterApplicationRestart(NULL, 0)` で crash / hang / patch / update reboot 全ケースの自動再起動を登録 (`restart_windows.go`)。非 windows は `nil` を返す no-op (`restart_other.go`、issue の「将来 OS 対応」フックを build-tag で表現)。`main.go` 起動時に 1 回呼び、失敗は best-effort で warn ログのみ。no-op 契約は `restart_other_test.go` (`//go:build !windows`) で Linux CI pin

パッケージ間依存: `state` / `imgdecode` 依存なし (imgdecode は x/image のみ)、`imgfile` → `logging` (非 Windows ビルドのみ、Trash fallback の warn log)、`thumb` → `imgdecode` + `imgfile`、`imgread` / `classification` → `imgfile`、`imghash` → `imgdecode` + `imgfile` + `logging`、`watcher` → `classification` + `imghash` (定数のみ) + `imgfile` + `logging`、`winhwnd` 依存なし、`winplacement` → `winhwnd` + `state` (型) + `logging` (windows ビルドのみ)、`winrestart` → `logging` (windows ビルドのみ)、`wintaskbar` → `winhwnd` + `logging` (windows ビルドのみ)。循環参照を作らない。

## 12. フロント feature 境界

- **`features/classification/`** ↔ `internal/classification`: 一覧 (分類) タブの UI 一式 + フィルタ / 配色 / グルーピング / コンテキストメニュー / watcher の純関数。`useGridThumbnail` で IntersectionObserver + module-scoped Map キャッシュ。`filters.ts` / `colors.ts` / `groups.ts` / `cardContextMenuLogic.ts` / `watcherPolicy.ts` / `thumbnailCache.ts` / `gridNav.ts` (矢印キーのグリッド移動先計算, #115) / `modalEditShortcuts.ts` (プレビュー編集の t/c/n フォーカス写像, #115) / `duplicateBadge.ts` (ダブりバッジ集合 / ペア除去, #136) / `sort.ts` + `sortMode.ts` (表示派生ソート + D-1 定数, #144) / `entriesEquivalent.ts` (watcher no-op gate の entries / fileTimes 等価判定) / `reorderEntries.ts` + `reorderMode.ts` + `useCardReorder.ts` の挿入計算 (並べ替えモード, #144 Phase 2) は vitest 対象。`useClassificationReorder.ts` (モード出入り + 並び替え保存 gate) は renderHook + IPC mock で検証`useDuplicateCheck.ts` (検出 kick / gen・folder・mode gate, #136) は renderHook + IPC mock で検証
...
- **`features/session/`** ↔ `internal/state`: 保存 / 復元グルー (`useSessionLoad` / `useSessionSave`) + `useWindowGeometryPolling` (window geometry / maximized polling, #86。Windows では `Environment().platform` で gate off し Go (`winplacement`) が window を単一所有、#129)。`useSessionSave` のみ `viewer-grid/layout` の型を import するクロス feature 依存を持つ (永続化は各 feature 状態を集約するため許容)
- **`features/settings/`** ↔ `internal/settings`: `useSettings` フック + `SettingsDialog` + セクション群 + `watchMode.ts` / `duplicateDetect.ts` (#136) 定数 (Go 側との D-1 同値テストで pin)
...
- **`App.tsx`**: 唯一の複数 feature オーケストレーション層。永続状態の hydration + 子フック (`useWindowGeometryPolling` / `useGlobalKeybindings` / `useViewerRename` / `useListToViewerHandlers` / `useViewerTabReorder` / `useSessionSave`) 組み立て + 子コンポーネント (`TopTabsBar` / `ClassificationView` / `ViewerGrid` / `SettingsDialog`) への配線 + ToastProvider / useConfirm 境界。`TopTabsBar.tsx` (上部タブ列) / `useGlobalKeybindings.ts` (Ctrl+Shift+1 が list、Ctrl+Shift+2..9 が viewer のため feature を跨ぐ) / `useTaskbarViewerSwitch.ts` (タスクバー ◀▶ の EventsOn 購読 → topTab + activeViewer 巡回, #149) は App-level として `src/` 直下に同居。`TopTab` 型 (`"list" | "viewer"`) は `src/topTab.ts`、taskbar イベント契約 (Go `wintaskbar` 定数の hand-mirror) は `src/taskbarEvents.ts` に独立
