...
- **#14 ハードコード値 → settings.json (実装完了)**: 4 種のハードコード値を settings に逃がして UI から変更可能に。schema bump なし (v1 に additive、per-field fallback 経路を流用)。
   - **`SettingsData` 拡張**: `MaxImagePixelsMP` (default 200) / `ThumbnailSize` (default 256) / `ThumbnailMode` (default letterbox) / `ThumbnailWorkerCount` (default 0 = NumCPU/2 auto) / `TagColors` (default `DefaultTagColors`) を追加。`Validate` / `applyFieldDefaults` に各フィールドの境界 (`isValidHexColor` 含む) を追加し、不正値は per-field 単位で黙ってデフォルトに戻す (タグ色マップは個別エントリ単位で drop)。Go テスト 11 → +5 ケース (`TestLoad_NewFieldsMissing_GetDefaults` で旧 v1 payload からの upgrade 経路を担保)。
   ...
   - **vitest +5 ケース** (`setKnownTagColors` / `getKnownTagColors`)、**Go テスト +5 ケース** (`isValidHexColor` 含む)。`tsc --noEmit` クリア、`wails build` 通過 (Linux ELF 5.281s)。
...
- **`internal/state`**: セッション状態の永続化 (v5 スキーマ: BSP ツリー `Layout` + Phase 1 leftover の `RootPath`/`LeftPaneWidth` を削除)。エクスポート: `StateData` ほか各 JSON 型 (`LayoutState` / `LayoutNodeState` / `TabState` / `ListTabState` / `WindowState`)、`Load()`、`Save(s)`、`DefaultData()`、`StateSchemaVersion`。依存なし。`LayoutNodeState` は `kind: "split" | "leaf"` 判別のタグ付きユニオン (Go では全フィールドを 1 構造体に持たせる方式)。
...
- **`internal/settings`** (Phase H1 + #14 拡張): ユーザー設定の永続化 (state とは別ファイル)。エクスポート: `SettingsData` (`LogLevel` / `MultiSelectMode` / `WheelMode` / `MaxImagePixelsMP` / `ThumbnailSize` / `ThumbnailMode` / `ThumbnailWorkerCount` / `TagColors`)、`Load()`、`Save(s)`、`Validate()`、`DefaultSettings()`、`SettingsSchemaVersion`、`MultiSelectCheckbox`/`Modifier`/`Both` 定数、`WheelModeZoom`/`ShiftZoom` 定数、`ThumbnailModeLetterbox`/`Crop` 定数、`DefaultTagColors`。`<UserConfigDir>/image-observer/settings.json` に書き出し、per-field fallback で壊れた値だけリセット (タグ色マップは個別 drop)。依存なし。schema は v1 のまま (additive 拡張)。
