...
- **A1 (推奨): Go 側は寸法 0 を返し、フロントが `<img>.naturalWidth/Height` で補完**
  - `decodeImageDimensions` の avif ケースは `return 0, 0, nil` (error にしない)。
  - `ImageView.tsx`: `<img>` の `onLoad` で `naturalWidth/Height` を読み、tab state の `imageWidth/Height` が未確定 (0) なら更新 → 初期 fit を駆動。
  - 長所: コンテナ解析不要 / 回転 (irot) 込みの **表示実寸** が常に正しい。
  - 短所: `preflight` の過大画像ガードが avif で効かない (§7 D4 で扱う)。フロント変更が要る。
...
- `ImageView.tsx`: `<img onLoad>` で `naturalWidth/Height` を読み、`tab.imageWidth/Height` が 0 のときだけ `onUpdateTabState` で補完 (既に Go 寸法がある形式は据え置き = 挙動非変更)。
- 既存の preview / original の src precedence・Blob revoke 責務 (spec D-9) には触れない。
...
- Go:
  - `imgfile`: `.avif` / `.AVIF` が `IsImage` true。
  - `imgread`: avif で `Read` が error にならず mime=`image/avif`、Data=元バイト列。寸法は採用方式どおり (A1: 0 / A2/A3: 期待値)。
  - `thumb`: avif で `Get` が error にならず元バイト列 + `image/avif` を返す (A 採用時)。
  - テスト用の最小 avif fixture を `testdata/` に置く (数百バイトの 1x1 等。生成手段は実装時に確定)。
- フロント (A1 採用時): `ImageView` の onLoad→naturalWidth 補完で初期 fit が走ること (純関数化できる部分は unit、難しければ手動確認に倒す)。
