...
| 2026-05-31 | PR #120 レビュー対応: §4.3 を 1 パス `summarizeTags` 中心に更新、§4.5 の props 記述 (`untaggedCount` は props でなく内部集計) を実装に追従、§4.5 に hydration 正規化を追記。 |
...
`filters.ts` に追加。`TagChips` はタグ別件数と未分類件数の両方を同時に要るので、
1 パスで両方返す `summarizeTags` を中心に置き、`tagSummary` / `untaggedCount` は
その薄いラッパとする (entry あたり `extractTags` は 1 回。PR #120 レビュー対応で導入):

```ts
export type TagSummary = { counts: Map<string, number>; untagged: number };

// summarizeTags: entry 列を 1 度だけ走査し、per-tag 件数 (counts) と未分類件数
// (untagged) を同時に返す。
export function summarizeTags(entries: classification.Entry[]): TagSummary { ... }

// tagSummary / untaggedCount は summarizeTags の薄いラッパ (API 互換):
...
initial state は `untaggedOnly: opts.initial?.untaggedOnly ?? false`。さらに復元値が
排他不変条件を破っている場合 (`untaggedOnly:true` かつ `tags` 非空) に備え、hydration で
`untaggedOnly` が true なら `tags` を空に正規化する (storage 形 → 表示形, AGENTS.md E-1)。

### 4.5 UI (`TagChips.tsx`)

- props を拡張: `untaggedActive: boolean`、`onToggleUntagged: () => void`。未分類件数は
  props で受け取らず、`entries` から `summarizeTags` で内部集計する (タグ別件数と 1 パスで同時に求める)。
...
- 配色: 「未分類」チップは Card の `(未分類)` バッジと揃えたグレー系で固定 (tagColor のハッシュ配色は使わない)。className は既存 `cls-chip` を流用し、未分類件数が 0 のときも表示するか非表示にするかは §8 D4。
