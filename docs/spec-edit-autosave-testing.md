...
| 2026-05-30 | Phase 2 (C) 着手 | as-built に確定: `SaveContext = { folder }` (D-2 a、mtime は fresh read で ctx に載せない)。ctx は SampleEditPane の `save` wrapper が dispatch 時に render-sync 済み `folderPropRef` から読む (キュー無改変)。§4.1 表の cleanup / queue 行、§4.2、§6.1〜6.3、§11 D-2 を実装に追従。 |
...
| unmount cleanup | `useEffect` cleanup | `entryRef` / `autoSaveRef` + `folderPropRef` | **folder** + mtime + dirty | ~~`ClassificationView.handleSave` の `folderPathRef` guard~~ (C で撤去) | **実装済み (#110 C)**: ctx.folder を `folderPropRef` 経由で固定し `saveEdit` で `ctx.folder !== folderRef.current` を pre-IPC skip。guard 撤去 |
...
| queue replay (dequeue) | `runSave` の `finally` | queued snapshot | **mtime** (前 save が mtime を advance) | `setTimeout(0)` + `loadResultRef.current.mtime` fresh read + ラッチ保持 (round 4) | **D-2(a) 採用済み**: `setTimeout(0)` + fresh read を温存 (ctx に mtime を載せない) |

### 4.2 folder と mtime は capture/dispatch の扱いが異なる (C の肝)

C で `ctx` を導入する際、**folder と mtime を同じ「capture 時固定」で扱うと round 2 の CONFLICT が再発する**。両者は意味が違うので、**ctx は folder のみを運び、mtime は saveEdit が fresh read** する (D-2 a):

- **folder = capture (= dispatch) 時に固定**。保存先はユーザーが編集した瞬間に確定し、後続 save が state を進めても変わらない。SampleEditPane が render-time sync する `folderPropRef` は、folder 切替で modal が unmount されると更新されない (= OLD のまま) ので、cleanup / queued replay が読む値は自然に OLD になる (§6.2 の等価性参照)。cleanup-after-switch では `ctx.folder = OLD` となり、saveEdit の **pre-IPC `folderRef.current(NEW) !== ctx.folder(OLD)` で IPC を撃たず skip** (NEW folder を汚さない / OLD folder にも書かない = 現 band-aid と同じ挙動)。これは現状 `onSaveRef.current` が OLD `handleSave` を握るのと同じ仕組みを **意図的に** 使う。
- **mtime = dispatch 時に解決** (ctx に載せない)。同一 folder への連続 save (queue replay) では、各 save 成功で sidecar mtime が advance する楽観ロックトークン。ctx に固定すると、queue replay の 2 本目が pre-save mtime を送り `CONFLICT:` を踏む (= round 2 で塞いだ穴)。よって saveEdit は pre-IPC gate を通過した = ctx.folder 上に居る前提で `loadResultRef.current.mtime` を読む。
...
D-2(a) 確定により **ctx は folder のみ** (mtime は saveEdit が fresh read するため ctx に載せない。§4.2):

```ts
// useClassificationEdit.ts で定義・export
export type SaveContext = { folder: string };
// SampleEditPaneProps / SampleModalProps
onSave: (next: classification.Entry, ctx: SaveContext) => void | Promise<void>;
```

### 6.2 SampleEditPane 側

- 新 prop `folder: string` を受け取り、**render-time sync** で `folderPropRef` に書く (AGENTS.md H-8「state ref の同期タイミング」)。`baselineMtime` は prop にしない (D-2 a)。
- ctx は **`save` wrapper が dispatch 時に `folderPropRef.current` から読む**: `onSaveRef.current(buildEntry(snap), { folder: folderPropRef.current })`。`runSave` / `useAutoSaveQueue` には手を入れない (キューは B のテスト網のまま挙動不変)。
- **dispatch 時読みでも OLD folder が保たれる理由**: folder 切替は必ず modal を unmount させる (`ClassificationView` の `useLayoutEffect([folderPath])` が `closeEdit`) ため、切替後に再 render されない `folderPropRef` は OLD のまま固定される。queued snapshot は `resetQueue` (entry?.filename 変化) で破棄されるので、dispatch が遅れても folder 切替を跨いだ replay は起きない。よって capture 時固定と等価 (§4.2)。

### 6.3 `useClassificationEdit.saveEdit`

- signature を `saveEdit(entry, ctx)` に変更。保存先を `folderRef.current` ではなく **`ctx.folder`** にする。
- pre-IPC で `if (folderRef.current !== ctx.folder) return;` (round 6 の置換、§4.2)。これにより cross-folder cleanup save は **IPC を撃たず skip** (現 band-aid と同じ挙動を、stale closure ではなく明示 ctx で実現)。
- mtime は §11 D-2(a) 採用: `ctx.folder` 上に居る前提で `loadResultRef.current.mtime` を fresh read (queue replay 整合性を温存)。`resolveConflictForce` は現 folder 上のユーザー操作なので `folderRef.current` のまま (folder 切替で conflict は `resetEntriesDependentState` がクリア)。
...
| ファイル | 対象 | Phase |
|---|---|---|
| `useAutoSaveQueue.test.ts` (`// @vitest-environment happy-dom`) | §5.4 の renderHook ケース全件 | 1 |
...
- **A 案 (採用・実装済み #110 C)**: `ctx` は folder のみ (`SaveContext = { folder: string }`)、mtime は `saveEdit` 内で `loadResultRef.current` から fresh read (現状の round 2 対策を温存)。`setTimeout(0)` dequeue も残す。最小変更で round 2/6 を両立。`baselineMtime` は ctx に載せない (未使用フィールドを避ける)。
