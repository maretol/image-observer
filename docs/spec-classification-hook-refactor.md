...
> **ステータス**: 実装完了 (PR #81)。§6 Q1-Q5 の確定事項は §0 改訂履歴に
> 記録。後続変更時は本書末尾の改訂履歴に 1 行追記してから着手すること。
...
| 2026-05-18 | 実装完了 (issue #66 / PR #81)。下記 2 点を実装中に確定: (1) §5-1 orchestrator 行数の想定が「~250-300 行」だったが、shared refs の宣言コメント + 戻り値 useMemo の deps array で実績は ~560 行。子フック側は最小 74 行 (filter) / 最大 557 行 (watcher)。(2) §5-3 / Q2 の props 型は「集約型 + `Pick<>`」案を **不採用** とし、各子フックが個別 `Props` 型を宣言する方針に確定。代案として§5-3 自身が言及していた「粒度が細かくなる分だけ追跡しやすい (型シグネチャに依存 ref が直接列挙される)」を優先。Pick は型レベルの遅延評価で grep しにくい欠点が実装中に顕在化したため。 |
| 2026-05-19 | PR #81 Round 1 (Copilot) 対応: 冒頭ステータスを「ドラフト」→「実装完了」に更新 (改訂履歴と整合)。実装には影響なし。 |
...
- 分割案:
  ...
  features/classification/
    useClassification.ts              // 本体 (orchestrator, 実績 ~560 行)
...
> **決定 (実装後)**: 集約型 + `Pick<>` 案は **不採用**。各子フックが個別
> `Props` 型を宣言する方針で実装。理由: 個別 props のほうが型シグネチャに
> 依存 ref が直接列挙され、grep / IDE の "Find Usages" でも追跡しやすい。
> Pick は型レベルの遅延評価で「`folderRef` を読む子フックを列挙」のような
> 確認がしにくいため。ボイラープレートは増えるが、各 ref が 12 個程度で
> 子フックあたり 5-10 props と現実的に収まった。

- (元案 1) 各子フックの引数を `Pick<ClassificationSharedRefs, "folderRef" | "requestGenRef" | ...>`
  で絞る方針。これで型シグネチャから「この子フックが依存する ref」が一目で
  分かる。
- (元案 2 = **採用**) 個別 props (子フックごとに別個の type 定義)。粒度が
  細かくなる分だけ追跡しやすい反面、ボイラープレートが増える。
