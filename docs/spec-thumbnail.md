...
`sem chan struct{}` (容量 = NumCPU/2) + `sync.Map` (inflight dedup: 同一 path 並行リクエストを 1 ジョブに合流)。

see `internal/thumb/thumb_pool.go`
...
`defaultDisplaySize=256`, `defaultMode="letterbox"`, `generateScaleFactor=2`, `jpegQuality=85`, `defaultWorkerCount=runtime.NumCPU()/2` を 1 ファイルにまとめ、将来 Settings 対応に差し替えやすくする。

see `internal/thumb/thumb_defaults.go`
