...
- 2026-05-19 Phase 1 実装完了 (PR #82)。`viewers.ts::moveViewer`、`useViewerSet.reorderViewer`、
...
- 2026-05-19 PR #82 ユーザー確認対応: drag 抑制条件を「自分が rename 中」から
  「**いずれかのタブが** rename 中」に拡張 (`anyRenaming` prop)。理由: 別タブの
  pointerdown で `preventDefault()` してしまうため input focus が維持され、rename
  editor を開いたまま兄弟タブが reorder できてしまっていた。
- 2026-05-19 PR #82 Copilot レビュー対応 Round 1: `ReorderState.ghostX` を削除
  (ghost 表示しない Phase 1 では未参照 + armed 段階の pointermove ごとに setState
  していた)。armed では setState せず、active でも insertIdx 変化時のみ setState
  するように最適化。
- 2026-05-19 PR #82 Copilot レビュー対応 Round 2: (a) pointercancel / Escape
  経路でも `justFinishedDragRef` を立て、active 終了後の trailing click を一律
  抑制 (commit のみだった非対称を解消)。`armSuppressClick` ヘルパに集約。
  (b) startDrag に `pointerId` を渡し、document リスナー側で一致 pointer のみ
  処理 (AGENTS.md H-2 マルチタッチ防御の厳密化)。(c) `.viewer-tab-insert-indicator`
  の margin コメントを「縦方向の高さ詰め」意図に修正。
...
ghost 位置 (clientX) は state に持たない (Phase 1 は ghost 表示しない方針)。
pointermove ハンドラは armed 段階 (= threshold 未満) では setState を呼ばず、active
段階でも `insertIdx` 変化時だけ setState することで、ドラッグ中の再レンダーを必要最小限に
抑える。
...
| armed | pointermove (< 5px) | state 更新なし (ghost 表示しない / insertIdx は srcIdx のまま no-op スロット) |
| armed | pointerup | 解放、style release。click が通常通り発火 → activate |
| armed | pointermove (≥ 5px) | `active` 昇格、insertIdx 更新 |
| active | pointermove | insertIdx 変化時のみ setState (同位置なら skip) |
| active | pointerup | `reorderViewer(srcIdx, insertIdx)` → release。**直後の click は 1 回だけ抑止** (§5.3) |
| active | Escape | release、配列不変、**trailing click を 1 回抑止** |
| active | pointercancel | release、配列不変、**trailing click を 1 回抑止** |

### 5.2 開始抑制条件 (drag を armed にしない)

- `viewers.length < 2` (並び替え対象なし)
- **いずれかのタブが rename 中** (`anyRenaming === true`)。自分自身が rename
  中のときは別 render path で onPointerDown ハンドラが描画されないので、ここで
  block されるのは「兄弟タブが rename 中」のケースが主。`preventDefault()` で
  input focus が維持されるため、これを許すと editor を開いたまま reorder できて
  しまう。
...
- **二重 pointerdown 防御** (AGENTS.md H-2): `stateRef.current` 既存なら即 return。
  加えて drag 中の document listener では `pointerIdRef` と一致しない pointer の
  pointermove / pointerup / pointercancel を ignore してマルチタッチ干渉を防ぐ。

### 5.3 click 抑制

active 状態で drag が終わったとき (commit / pointercancel / Escape の **いずれでも**)、
直後にブラウザが発火する click を握り潰す:

- `armSuppressClick()` で `justFinishedDragRef.current = true` をセット
...
旧 active が戻る挙動になりうる。pointercancel / Escape では synthetic click が出ない
ことが多いが、出た場合の UX 不一致を避けるため commit と同じく握り潰しておく
(commit と cancel で挙動を非対称にしない)。1 回分の click を捨てるのは DnD UI として
一般的。armed-only (= threshold 未到達で離した) 経路では立てないので、通常 click の
activate には影響しない。
...
| 11.6 | drag 中の click 抑制 | active 終了 (commit / pointercancel / Escape) のいずれでも `armSuppressClick()` で 1 click だけ握り潰す。次 rAF で flag クリア。commit と cancel で挙動を非対称にしない |
| 11.6b | マルチタッチ防御 | `startDrag` で `pointerId` を保存し、document リスナーは一致 pointer のみ処理。AGENTS.md H-2 の二重 pointerdown 防御 + concurrent pointer の干渉防御 |
| 11.7 | 1 件のときの動作 | `viewers.length < 2` のとき pointerdown を drag に変換しない |
| 11.8 | rename 中の動作 | **いずれかのタブが rename 中** (`anyRenaming === true`) のとき drag を開始しない。自分自身の rename は別 render path だが、兄弟タブ pointerdown も `preventDefault()` で input focus を温存してしまうため両方を block する |
