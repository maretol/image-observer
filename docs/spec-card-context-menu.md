...
> **ステータス**: ユーザー合意済み (2026-05-16)、実装着手可。
...
viewers.length === 1 のときは「ビューア「{name}」で開く」項目 1 個 + divider なしで OK。
...
- **vitest (純関数 / 軽量 component)**:
  ...
  - メニュー項目組み立てロジック (viewers の数 / mode による分岐) を component-light なテストでカバー (`render` + `getByRole("menu")` 程度)。
- **Go テスト追加なし** (IPC 変更なし)。
- **手動確認 (test plan)**:
...
- viewers.length === 1 のとき: 単一モードで「ビューア「{name}」で開く」のみが表示される (divider なし)
...
### D. 選択外カードを右クリック — selection 不変 (§11-B)
...
| `frontend/src/features/classification/CardContextMenu.test.tsx` (新規) | mode 決定ロジック + 項目組み立てのテスト (vitest) |
