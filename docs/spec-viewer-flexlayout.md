...
- `SplitDirection`: `"row"` (上下) | `"col"` (左右)
- `SplitNode`: `{ kind:"split", id, direction, ratio (0-1), a, b: LayoutNode }`
- `LeafNode`: `{ kind:"leaf", id, tabs: Tab[], activeIndex }`
- `Layout`: `{ root: LayoutNode, activeId: string }`
- `Tab` 型は Phase 3a `useTabs.ts` のものを再利用

see `frontend/src/features/viewer-grid/layout.ts`
...
1 パネル (空 LeafNode) + `activeId` = そのルート leaf の id。

see `frontend/src/features/viewer-grid/layout.ts:initialLayout`
...
主要 API: `openInActive(path)` / `setActivePanel(leafId)` / `setActiveTab(leafId, idx)` / `closeTab(leafId, idx)` / `updateTabState(leafId, idx, patch)` / `moveTabIntoLeaf(srcId, srcIdx, dstId, dstIdx?)` / `reorderTab(leafId, srcIdx, dstIdx)` / `splitTabIntoEdge(srcId, srcIdx, dstId, edge)` / `setSplitRatio(splitId, ratio)`

see `frontend/src/features/viewer-grid/useViewerGrid.ts`

### 6.3 純関数 (layout.ts) の責務分離

ツリー操作はすべて純関数として `layout.ts` に切り出し、`useViewerGrid` の `setLayout(prev => transform(prev, ...))` から呼ぶ:

主要純関数: `findLeaf` / `findParent` / `enumerateLeaves` / `replaceNode` / `collapseEmptyLeaf` / `updateLeaf` / `updateSplit` / `validateLayout`
...
`RenderNode` が `leaf` → `<Panel>` に、`split` → `<div flex>` + `<GridSplitter>` + 再帰 `<RenderNode a/b>` に変換する再帰描画。CSS Grid ではなく **Flexbox の入れ子** で実装 (BSP ツリーとの相性)。

see `frontend/src/features/viewer-grid/ViewerGrid.tsx`

## 7. セッション復元

### 7.1 Go state スキーマ (state.go)

`StateSchemaVersion = 4`。`StateData.Grid GridState` を `Layout LayoutState` に置換。`LayoutState = { Root LayoutNodeState, ActiveID string }`. `LayoutNodeState` はタグ付きユニオン (`Kind: "split"|"leaf"`) で SplitNode/LeafNode 両フィールドを持つ。v3 以前は version 不一致で default fallback (マイグレーションなし)。`validateState` で ratio 範囲クランプ / activeId 不在時は DFS 先頭 leaf にフォールバック。
