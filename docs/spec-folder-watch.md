...
| ディレクトリ | Create | `os.Lstat` で symlink を弾いてから配下を `filepath.WalkDir` で再帰 enumerate → 各サブフォルダを `w.Add` + 既存画像ファイル数を `addedFiles` に集計 (`anyChange = true`)。**symlink (target を follow しない)** = `anyChange = true` のみ立て、target tree への watch 拡大を防止 (分類スキャナも symlink を辿らない設計と整合、「現在フォルダのみ監視」の前提を保つ)。 |
