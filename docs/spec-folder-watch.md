...
| **changed イベント** | watcher が debounce / coalesce 後にフロントへ emit する集約イベント。`{ folder, addedFiles, removedFiles, renamedFiles, contentChanged, sidecarChanged }` を含む。 |
...
type ChangedPayload = {
  ...
  contentChanged: boolean; // 既存画像への Write があった (同名上書き。ダブり再判定 kick 用, spec-duplicate-detection §8.1)
...
外部変更を silently 上書きする重大バグになる。実装は [watcherPolicy.ts](../frontend/src/features/classification/watcherPolicy.ts) の `decideAutoMerge`。
...
**自己 echo の抑制**: アプリ自身が呼んだ sidecar / 画像書き換え系バインディング (`SaveClassification` / `UpdateClassificationEntry` / `CreateEmptyClassification` / `MergeChildSidecars` / `DeleteImage`) も watcher に拾われて `classification:changed` を発火する。フロント側ハンドラは fresh の entries (filename / folder / confidence / note の順序込み深比較) と現在の `loadResult.entries` の等価判定で自己 echo を分岐する:

- entries 等価 + mtime 等価 → **完全 no-op**
- entries 等価 + mtime のみ違う → **静かに commit** (loadResult.mtime のみ更新、次回 save の expectedMtime を新鮮に保つ)
- entries が違う → 通常フロー (toast + `decideAutoMerge`)

`Service.Load` の戻り順序 (sidecar 順 → アルファベット順) は決定的なので順序込み比較で安全。

**`DeleteImage` の self-echo 抑制 (asymmetric strip)**: entries 比較だけでは sidecar save の完了タイミングに依存して取り逃すため、`inFlightDeletesRef: Map<folder, Set<filename>>` で削除中 filename を folder スコープで保持する (`deleteOne` 冒頭 add / finally で delete)。watcher handler と `silentRecheckAfterStart` の等価判定では **cur 側 (削除前の stale な現在表示) だけ** in-flight filename を strip して fresh と比較する。fresh 側を strip しないのは、削除 IPC のラウンドトリップ中に外部で同名ファイルが再作成されたとき、その再作成エントリを strip すると差分を取り逃して silently 隠してしまうため。Map の key を folder にしているのは、フォルダ切替後に旧 folder で進行中の delete filename が新 folder の strip に混入する race を避けるため。
...
フロントが Start 完了後の stale 判定で `dispatchWatchIntentRef.current()` を再 dispatch しても、Go 側で重複 walk は走らない。`useClassification` には `tryStart` のような個別ヘルパは持たず、Start/Stop は §6.3 の単一導管 `dispatchWatchIntentRef` に集約する。
...
`runtime.EventsEmit(a.ctx, watcher.ClassificationChangedEvent, ChangedPayload{...})` を呼ぶのは **`app.go` だけ** (`NewApp` で `watcher.NewManager` に渡す `EmitFunc` コールバック内で結線)。event 名はリテラルではなく `watcher.ClassificationChangedEvent` 定数で参照する — Go 側 (`TestClassificationChangedEventName`) と TS 側 (`watcherPolicy.test.ts`) の D-1 ドリフト検知テストで同値を確認している。

`internal/watcher` は `wails/v2/pkg/runtime` を import せず、`Manager.emit(payload)` (DI 受領関数) を呼ぶだけ。これによりテストで `EmitFunc` を差し替え可能 (§11) + 将来 IPC 機構が変わっても影響範囲を app.go に閉じ込められる (§7.3 / §12)。

`ChangedPayload` は Wails の `generate module` で TS 型を自動生成できない (EventsEmit payload は binding シグネチャに現れない) ため、フロントは §4.2 のとおり [watcherPolicy.ts](../frontend/src/features/classification/watcherPolicy.ts) で hand-mirror する (リネーム時は Go / TS 両方を同時更新)。

### 6.3 ライフサイクル責務

`useClassification` 内の `StartFolderWatch` / `StopFolderWatch` 呼び出しはすべて単一の `dispatchWatchIntentRef` 経由で行う (folder-watch effect が `folderPath` / `watchMode` の変化を見て dispatch する単一導管)。`openFolder` / auto-load effect / 設定 UI から直接 Start / Stop を呼ぶ経路は持たない — JS から並列で Start("A") → Start("B") を発行すると Wails の per-call goroutine dispatch によって Go 側 `m.mu` 取得順が逆転し得るため、最後の IPC が "勝者" になる race を避ける目的 (fixed-point 収束の主要設計点)。

各経路の責務:

| 経路 | 責務 |
|------|------|
| `openFolder` / auto-load effect | `folderPath` / `folderRef` を更新するのみ。Start/Stop は folder-watch effect が処理 |
| WatchMode 切替 (`"auto"` ↔ `"off"`) | folder-watch effect が依存変更で再 fire、intent に応じて Start/Stop 発行 (Go 側は設定値を知らず「呼ばれたら起動する」スタンス) |
| Start/Stop IPC 完了 (success/error) | `dispatchWatchIntentRef` の then/catch で現 intent を再評価、ズレていれば再 dispatch (Start same-root = no-op / Stop = idempotent 前提) |
...
起動シーケンス: `os.Lstat(root)` でディレクトリ+symlink でないことを確認 → `w.Add(root, All)` (失敗なら error 返却) → `filepath.WalkDir` で全サブフォルダを列挙し `w.Add`、隠しフォルダは除外 (失敗は warn して続行 / best-effort)。

see `internal/watcher/watcher.go:Manager.Start`
...
で `ev.Name` および配下の watch path を一括解除** する。単一の
...
| 画像ファイル | Write | **counter は bump せず `contentChanged = true` を立て、debounce timer を延長** (`triggered=true`)。大きい画像のコピー (Create → Write → Write → …) で Create だけ拾って 200ms 後に flush すると、フロントが未完成 / 0-byte ファイルを開いてサムネエラーを出す race を避けるため、Write 中は quiet window を伸ばし続ける。Write 単独 (既存画像の上書き) の flush はかつて no-op だったが、ダブり検出 (#136) が同名上書きの再判定 kick を必要とするため `contentChanged` のみの payload を emit する — entries 等価 + mtime 等価ならフロントは silent に落ちるので toast / commit は発生せず、「既存画像の content edit はサムネ更新されない」挙動も従来どおり (`useGridThumbnail` の path-key cache 制約は §13.14)。 |
...
| 画像拡張子のディレクトリ (`photos.jpg/`) | Remove / Rename | `watchState.watchedDirs` に存在すれば dir 削除と判定し `anyChange` のみに倒す (`removedFiles++` しない)。`acc.removedPaths` で IN_DELETE + IN_DELETE_SELF + IN_IGNORED の多重発火を per-window dedup。 |
...
| 画像拡張子の symlink | Create | image-Create branch に fall through (`addedFiles++`)。分類スキャナは Lstat ベースで extension のみ判定するため symlink でも entries に含めるので、payload の `addedFiles` カウントを再 Load 結果と一致させる必要がある。 |
| ディレクトリ | Remove / Rename | `removeSubtreeFromWatch(st, ev.Name)` で `ev.Name` 配下の watchedDirs + w.Remove を一括解除 + `anyChange = true`。Linux inotify は rename で watch を inode に残すため、descendant も含めて解除しないと移動先のサブツリー変更が現在 root の `classification:changed` に流れ込む。base に画像拡張子も sidecar 名も付かない Remove / Rename は dir or 非画像ファイル相当として扱い、re-Load トリガとして十分 |
...
`loop` goroutine で `st.watcher.Events` / `.Errors` / `st.stop` を `select` で待つ。イベント到着で `pending.accumulate` → `timer.Reset(200ms)`。200ms 静止後に `pending.snapshot` → `emit` (flush)。`Errors` は `anyChange=true` にして debounce 継続。`Stop` 到着で `pending` を discard して return。

see `internal/watcher/watcher.go:Manager.loop`
...
主要 API: `NewManager(emit EmitFunc) *Manager` / `NewManagerWithDebounce(emit, d)` (テスト用) / `Start(root) error` / `Stop() error` / `Current() string`。

see `internal/watcher/watcher.go`
...
新規 watcher を構築する経路で復旧する。

明示的 Stop で同じ分岐に来た場合 (`stopRequested == true`) はログも flush も
せずに即 return する — Stop の意図は「いま積んでいる変更は捨てる」だから
(§7.3)。

「死亡通知イベントを emit してフロントから再 Start させる」案は不採用 — watch 死亡は環境要因 (max_user_watches 不足等) で再試行しても治らないことが多く、フロント側ロジックも複雑化するため。Phase 1 は「フォルダ切替 or 手動 reload で復活」で十分。
...
気付かず一覧が古いまま残るのを避けるための保険。次回 flush で
...
logger.debug("watcher", "flush", { folder, added, removed, renamed, content, sidecar })   // 集約 flush 時 (ChangedPayload と同じフィールドを必ず含める)
```
...
| 区分 | ケース概要 |
|------|------------|
| Start / Stop 契約 | 欠落 root で fail、**file 渡し root で fail**、同一 root 再 Start は no-op、folder 切替で旧 watch を tear down、root vanish 後の Start 再構築 |
...
| 画像内容変化 | 既存画像への Write のみの window でも `contentChanged: true` の payload が emit される (同名上書き、#136) |
| 拡張 / dir 挙動 | サブフォルダ Create で watch 拡張 + 配下ファイル Create が拾える、subtree rename-out で descendant watch が一括解除される |
| symlink / 名前 race | dir symlink は target を follow しない、画像拡張子 symlink は `addedFiles` に計上、画像拡張子の dir (`photos.jpg/`) Remove は `anyChange` のみで `removedFiles++` しない |
