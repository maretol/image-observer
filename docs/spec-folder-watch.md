...
| ディレクトリ | Create | `os.Lstat` で symlink を弾いてから配下を `filepath.WalkDir` で再帰 enumerate → 各サブフォルダを `w.Add` + 既存画像ファイル数を `addedFiles` に集計 (`anyChange = true`)。walk で見つけた画像 path は `acc.discoveredImagePaths` 共有 dedup set にも登録し、**parent walk と child Create event 両方で同じ画像を見つけた race / 並行 inotify Create vs walk の race の double-count を防ぐ** (新しい path のみ addedFiles に加算; PR #75 12th)。 |
