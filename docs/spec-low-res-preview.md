...
> **ステータス**: ドラフト。§10 の決定事項をユーザー合意後に着手。
...
- [frontend/src/features/classification/SampleModal.tsx](../frontend/src/features/classification/SampleModal.tsx#L18): `PREVIEW_SIZE = 1024` / `PREVIEW_MODE = "letterbox"` で `GetThumbnail` 呼び出し。**この定数を流用する**。
...
| D-14 | 既存の "読み込み中…" テキスト | **preview / 寸法のいずれも未到着の間だけ表示**。寸法到着 → `<img>` レンダリング開始 (src は preview or original or 空文字) | 寸法 + (preview or original) の AND が成立した時点で `<img>` を出す。**寸法のみ + src 無し** だと一瞬空 `<img>` (alt 表示) が出るのを避けるため、テキストは「寸法 OR src の片方でも未到着の間」表示にする |
...
4. **D-15 (preview を出さない場合の挙動)**: **寸法到着前は "読み込み中…" 維持、寸法到着後は空 `<img>` のまま original 待ち** (D-14 と整合)。
