...
| 2026-05-29 | #106 反映: §11 Out of scope の「viewer 横断のプレビューメモリキャッシュ」を Phase 1 化したため、対応する spec へのリンクを §10-5 / §11 に追記 ([spec-viewer-tab-cache.md](spec-viewer-tab-cache.md))。本仕様自体の挙動変更はなし。 |
...
| D-7 | プレビューとオリジナルの寸法不一致時の `<img>` style | **original 表示中は `width = imageWidth, height = imageHeight` (元画像寸法)。preview 表示中は `width = height = max(W, H)` の正方形にして、`translate3d` に `(W - max)/2` / `(H - max)/2` の offset を加え、letterbox 余白をコンテナ `overflow:hidden` の外側へ押し出す。content 領域は両モードで同じ `[0..W]×[0..H]` 矩形に乗る** | 初版 (PR #103) は preview も元画像寸法で render していたが、N×N letterbox bitmap を非正方形に直接 stretch すると非一様 scale で content のアスペクト比が崩れる視覚バグ (#104) が出た。content 領域を矩形にぴったり載せる構成に切替えても、bitmap の透過余白は container 外側にしか出ないので追加描画コストはなし。switch は React の単一 render で commit されるので preview→original 移行時の中間 frame は出ない |
...
- `<img>` の `width / height` / 描画位置 (D-7):
  - **original 表示中** (`imageData !== null`): `width = tab.imageWidth, height = tab.imageHeight`、translate3d は `(panX, panY)` のみ
  - **preview 表示中** (`imageData === null && previewUrl !== null`): `width = height = max(tab.imageWidth, tab.imageHeight)` の正方形 + `translate3d(panX + (W-max)/2 * zoom, panY + (H-max)/2 * zoom, ...)` で letterbox 余白をコンテナの外側へ寄せる
  - 寸法 (`tab.imageWidth/Height`) は GetImageInfo / ReadImage の先着で埋まる前提
...
3. **D-6 (transition)**: **入れない**。1 frame の瞬間置換 (D-7 の preview/original 両モードとも content 領域は同じ `[0..W]×[0..H]` 矩形に乗るので位置 / サイズはズレない)。
...
5. **Phase 2 候補**: 今回は Phase 1 のみ実装。Phase 2 用の issue 起票は使ってみて判断する (skip threshold / fade transition / viewer 横断メモリキャッシュ等)。 → **viewer 横断メモリキャッシュは #106 で Phase 1 化済み** ([spec-viewer-tab-cache.md](spec-viewer-tab-cache.md))。残る Phase 2 候補は skip threshold / fade transition 等。

## 11. Out of scope (Phase 1 では作らない)

- preview / original の placeholder としての CSS animation (skeleton 等)
- ~~viewer 横断のプレビューメモリキャッシュ (タブ閉じ → 再オープン時の即時表示)~~ → **#106 で Phase 1 化済み** ([spec-viewer-tab-cache.md](spec-viewer-tab-cache.md))
