...
| 2026-06-21 | 実装に合わせ追補 | セルフレビューで spec と実装を突合: D1 のタイトル一致タイブレークは未実装に確定 (4 条件で実質一意 + D-1 ドリフト回避)。§8 に WebView 破棄で Go が最終 writer になる旨を追記。 |
...
- タイトル文字列 (`"Imago"`) への依存を**避ける** (将来 `WindowSetTitle` しても壊れない上、
  `main.go` の Title 定数を winplacement 側に複製すると AGENTS.md D-1 のドリフトを生むため)。
  ドラフト段階では「複数候補時のタイブレークにタイトル一致を補助使用」を検討したが、
  上記 4 条件 (own-PID + visible + owner==0 + caption>0) で実質一意になるため**実装では
  タイトル一致を使わず最初の該当 HWND を採る** (D1)。
...
- 通常終了で経路 4 が経路 2 の後に発火する懸念: ウィンドウが閉じると WebView が破棄され、
  保留中の JS タイマー (useSessionSave の debounce) はキャンセルされるため、Go の
  `OnBeforeClose` が事実上の最終 writer になる。閉じる直前に state 変更が無ければ保留保存も
  無いので、ほぼ全ケースで Go が勝つ。
...
- **D1 — HWND 取得**: `EnumWindows` + 自プロセス PID マッチ (+ 可視 + owner==0 + 非空タイトル)。
  ...
  実装では検討していたタイトル一致タイブレークも**使わない** (4 条件で実質一意 + D-1 ドリフト回避、§3.2)。
