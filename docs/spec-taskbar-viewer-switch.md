...
| 2026-07-20 | /code-review 反映 | マルチエージェントレビューの確定指摘を反映: THB_ICON 定数 0x2 修正 / origWndProc を swap **前**に atomic 公開 (§3.2) / OnStartup は window 表示 (navigationCompleted) より先に走るため Setup に HWND 探索 retry を追加 (§3.2, D7 改訂) / ThumbBarAddButtons の初回失敗を warn に (§10) / アイコン読み込みは LookupIconIdFromDirectoryEx でなく ICONDIR 自前走査 (§5.3 追従) / FE gate を App.tsx `tabSwitchGated` に一元化 (§5.2) / `cycleViewerId` の「切り替え不要」を null に統一 (§11.2)。 |
...
- **旧 wndproc の公開順序** (/code-review 反映): swap の**前**に `GetWindowLongPtrW` で旧 proc を
  取得し `atomic.StoreUintptr` で公開してから swap する。`SetWindowLongPtrW` の戻り値を後から
  代入する方式は、カーネル内で swap が完了した瞬間〜代入可視化の隙間に UI スレッドが
  `CallWindowProcW(NULL)` (未定義動作) を踏む race を作る。wndProc 冒頭の atomic load が
  store と対になり、先行 write (onSwitchCb 等) の可視性も保証する。
- **HWND 探索の retry** (/code-review 反映): Wails は window を navigationCompleted (WebView2 の
  ロード完了) まで**表示しない**一方、`OnStartup` は goroutine でそれより先に走るため、初回の
  `FindMainWindow` は `IsWindowVisible` 条件で失敗しうる。`Setup` は初回失敗時に
  250ms × 40 回 (最長 10 秒) のバックグラウンド retry を行い、上限到達時のみ warn する
  (retry は単一 goroutine 内で直列 = 二重装着なし)。
- **スレッド制約**: `Setup` は `OnStartup` から呼ばれ、UI スレッドである保証がない
  (実際 Wails v2.13 は OnStartup を goroutine で起動する)。
...
//   Setup(onSwitch func(direction string)) (ok bool)  // windows: 配線開始 (HWND は window 表示待ち retry) / other: false
```
...
| 設定ダイアログ表示中 / 一覧の並べ替えモード中 | **no-op**。gate 条件は App.tsx の `tabSwitchGated` 1 箇所で管理し `useGlobalKeybindings` と共有 (フックごとの列挙は次のモーダル追加時に drift するため) |
...
- ◀ / ▶ の矢印 2 個を **.ico ファイル (16/20/24/32px マルチサイズ、HiDPI スケール対応) として
  commit** し (`internal/wintaskbar/assets/`)、`go:embed` → **ICONDIR を自前走査**
  (`pickIconEntry`: `SM_CXSMICON` に最も近い幅のエントリを選択) → `CreateIconFromResourceEx`
  でメモリから `HICON` 化する。`LookupIconIdFromDirectoryEx` は**使えない** — 同 API が期待する
  リソース形式 (GRPICONDIR、エントリ末尾が nID uint16) と .ico ファイルの ICONDIR (エントリ末尾が
  dwImageOffset uint32) はレイアウトが異なるため。
...
|---|--------------|---------|----------------|----------------|-----------|
| 1 | Go `Setup` (OnStartup) + retry goroutine | 起動時 1 回 + 初回失敗時 250ms × 40 回 | HWND (`winhwnd.FindMainWindow`) | **window は navigationCompleted まで不可視**のため初回はほぼ確実に未発見 | 単一 goroutine 内で直列 retry (二重装着なし)。上限到達のみ warn。旧 wndproc は swap **前**に atomic 公開 (§3.2)。subclass 装着後の初期登録は PostMessage で経路 2 へ委譲 (Setup 自身は COM を触らない) |
| 2 | wndproc: `WM_APP+1` / `TaskbarButtonCreated` (UI スレッド) | 起動時 + explorer 再起動時 (回数不定) | ITaskbarList3 instance (module state, lazy init) | explorer 再起動でタスクバー側の登録が消える = 「登録済み flag」は stale になる | flag で抑止**しない** — 受信のたびに `ThumbBarAddButtons` を再実行 (冪等化)。ログレベルのみ `buttonsRegistered` (UI スレッド専有) で分岐: 初回成功前の失敗は warn / 成功後の再登録失敗は debug (§10) |
...
| 事象 | 挙動 |
|------|------|
| HWND が見つからない | 250ms × 40 回 retry (window 表示待ち、中間失敗は debug)。上限到達で warn + 機能無効のまま起動継続 (best-effort) |
| `GetWindowLongPtrW` / `SetWindowLongPtrW` 失敗 (戻り値 0) | warn + その回の trySetup を失敗扱い。subclass 未装着なので以降のメッセージ処理も発生しない |
| `CoCreateInstance` / `HrInit` 失敗 | warn + ITaskbarList3 を nil のまま (次の TaskbarButtonCreated で再試行) |
| `ThumbBarAddButtons` が失敗 | **初回成功前は warn** (登録が一度も成っていない = 実障害の可能性)。一度成功した後の再登録失敗は debug (経路 2 の冪等化の正常系) |
...
- `cycleViewerId(viewers, activeId, direction)` 純関数 (viewers.ts に追加): wrap-around /
  activeId 不明時は先頭へフォールバック / **「切り替え不要」(空・1 個・移動先が現在と同一) は
  null** (呼び出し側は null チェックのみで same-id guard の暗黙契約を持たない)、を検証。
...
- **D6 — アイコン**: ◀▶ の .ico 2 個を commit + `go:embed` + `CreateIconFromResourceEx`。
  実行時 GDI 描画はしない。
- **D7 — best-effort**: subclass 失敗 / COM 失敗は warn して機能無効で起動継続
  (winrestart.Register と同じ扱い)。**HWND 探索のみ例外的に retry する** (/code-review で改訂:
  window が navigationCompleted まで不可視なため、リトライなしでは通常起動でほぼ確実に失敗する。
  250ms × 40 回で打ち切り、上限到達時のみ warn)。
