...
├── taskbar_other_test.go          // //go:build !windows: no-op 契約を Linux CI で pin (#133 流儀)
└── assets/prev.ico, next.ico      // go:embed するボタンアイコン (§5.3)
```
...
- ◀ / ▶ の矢印 2 個を **.ico ファイル (16/24/32px マルチサイズ) として commit** し
...
- [internal/winplacement/placement_windows.go](../internal/winplacement/placement_windows.go) —
  `findMainWindow` (winhwnd へ移設する対象) / syscall.NewCallback / LazyDLL の手本
