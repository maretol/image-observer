...
- [ ] **タスクバーからのビューアタブ切り替え (#149)**: Windows タスクバーアイコンのサムネイルプレビューに `ITaskbarList3` サムネイルツールバーで「前/次のビューア」2 ボタンを置き、ウィンドウ非フォーカスのままビューアタブを巡回 (液タブのマウスオーバー操作向け、2026-07-20 ユーザー合意)。ジャンプリスト (タブ名直接選択) は単一インスタンス転送が要るため Phase 2 の別 issue に分離。Go 側は `internal/wintaskbar` (windows/other build-tag) + window subclass で WM_COMMAND を捕捉し EventsEmit、FE は keybindings と同じ gate で activeViewer を巡回。永続化変更なし。ci.yml に `GOOS=windows go build` のクロスコンパイル check も追加 (D8)。実装済み (feat/149-taskbar-viewer-switch)、Windows 実機のボタン動作確認は PR test plan 持ち、チェックは PR マージ後。 → [spec-taskbar-viewer-switch.md](spec-taskbar-viewer-switch.md)
