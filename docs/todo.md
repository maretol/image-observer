...
- [ ] **タグ未付与フィルタ (未分類だけ表示, #116)**: 一覧タブのタグチップ行に「未分類」チップを追加し、`extractTags(folder)` が空のエントリだけに絞る。triage 合意 = 独立 boolean `untaggedOnly` + 通常タグとの排他モード。表現方法を sentinel ではなく独立フィールドにしたのは永続化される `tags[]` の意味を濁さないため。**schema バンプ要否 (D1) は検証の結果「無バンプ additive で前方/後方互換」が判明し推奨 = 据え置き** (v6→v7 にすると migration 必須・既存 state 消失リスク)。 → [spec-untagged-filter.md](spec-untagged-filter.md)
