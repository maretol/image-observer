...
- [ ] **キーバインド**: タブ移動 (Ctrl+Tab / Ctrl+PgUp/Dn) / タブクローズ (Ctrl+W) / ズームリセット (Ctrl+0 or Double click) / 原寸 (Ctrl+1) / フィット (Ctrl+F) — 採用するセットを決める。Phase 5 で持ち越した Esc キャンセル (DnD 中) もここで扱う。Phase 4 v1.4 で導入した「複数選択ホットキー (Ctrl+クリック / Shift+クリック)」も合わせて検討。
  - 結論:
- [ ] **複数選択 UI のオプション化**: 一覧タブの複数選択は v1 では「常時 checkbox 表示」(案 A) で実装済み。将来の追加案として **修飾キー併用** (案 B: Ctrl+クリック でトグル / Shift+クリック で範囲選択) を設定で切り替え可能にする。設定キー候補: `list.selection.mode = "checkbox" | "modifier" | "both"`。
  - 結論:
- [ ] **テーマ**: ダーク固定 / ライトも用意。
  - 結論:
- [ ] **言語**: 日本語固定 / 英日切替。
