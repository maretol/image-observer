...
- [x] **タブの順序操作 / パネル分割**: タブ並び替え (DnD で順序変更) は v1 不要。ただし **ビューア領域を最大 6 ペイン (2 行 × 3 列のグリッド) に分割し、タブをペイン間で移動** できる仕組みは v1 で実装する。
  - 結論:
    - **タブ並び替え** (パネル内での順序入れ替え): v1 では実装しない。後回し。
    - **パネル分割**: 右ペイン (ビューア領域) を最大 `2 行 × 3 列 = 6 パネル` のグリッドに分割可能。
      - 既定状態は 1 行 × 1 列 = 1 パネル。
      - ユーザー操作 (ボタン or ショートカット) で行追加 / 列追加 / 行削除 / 列削除を行う。
      - 各パネルは独立した TabBar とアクティブタブを持つ。
      - パネル間のサイズはスプリッター (行間 / 列間) でドラッグ調整可能。
      - **アクティブパネル** の概念を持つ (直近にフォーカスされたパネル)。ツリーからの画像オープンはアクティブパネルに開く。
    - **タブ移動**: あるパネルで開かれているタブを、別の任意のパネルに移動できる (右クリックメニュー or 専用コマンド経由、DnD は v1 不要)。
    - **空パネル削除時の挙動**: タブが残っているパネルを削除しようとしたら確認 or 隣接パネルへタブをマージ。詳細 UX は spec で詰める。
  - **重要 — init.md §2.3 の仕様変更**: init.md §2.3 では「ペインの 3 分割以上、ドッキング、フローティング」をスコープ外と明記しているが、この **「3 分割以上」を取り下げ、最大 6 ペインのグリッド分割を v1 スコープに追加** する。「ドッキング、フローティング」は引き続きスコープ外を維持 (= グリッド分割のみ、フローティングウィンドウは作らない)。理由はユーザー判断 (画像比較ワークフローのため)。
  - **将来対応**: グリッド最大数 (現 2 行 × 3 列) は固定値 `MAX_ROWS / MAX_COLS` で定数化する。Phase H で設定画面 UI から変更できるようにする。コード上は `useViewerGrid.ts` 冒頭の定数を 1 箇所にまとめ、将来 Settings 由来の値に差し替える設計とする (Phase 2 サムネ既定値の `thumb_defaults.go` と同じパターン)。
...
- [ ] **壊れた画像**: タブ内にエラー表示 / タブを開かない / トースト通知。
  - 結論:
- [ ] **巨大画像**: サイズ上限 (例 200MP) を超えたら開かない / 縮小プレビューのみ。
  - 結論:
- [ ] **アクセス権なしフォルダ**: スキップしてログ / ユーザに通知。
  - 結論:
- [ ] **フォルダ消失中の挙動**: 再描画 / エラー表示。
  - 結論:

## H. UX / ショートカット 🟢

- [ ] **キーバインド**: タブ移動 (Ctrl+Tab / Ctrl+PgUp/Dn) / タブクローズ (Ctrl+W) / ズームリセット (Ctrl+0 or Double click) / 原寸 (Ctrl+1) / フィット (Ctrl+F) — 採用するセットを決める。
  - 結論:
- [ ] **テーマ**: ダーク固定 / ライトも用意。
  - 結論:
- [ ] **言語**: 日本語固定 / 英日切替。
