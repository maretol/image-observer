...
- [x] **テスト**: Go ユニットテストとフロント vitest を導入済み。
  - 結論: Go は `internal/` 配下の各パッケージ (classification / imgread / thumb / state / settings / logging / imgfile / watcher) でテーブルテスト中心、加えて root の `app_test.go` (`App.DeleteImage` パスバリデーション)。フロントは vitest で純関数 (filters / colors / groups / layout / logger / keybindings / watcherPolicy / watchMode / cardContextMenuLogic ほか) を中心に展開。CI は ci.yml で `go test ./...` + `npm run test` + `tsc --noEmit` を `pull_request` (branches: main) + `workflow_dispatch` トリガで実行 (件数は追加のたびに動くので最新は `go test ./...` / `npm --prefix frontend test -- --run` の出力を参照、AGENTS.md A-1)。ローカルで件数を確認する場合は CI と同じく事前に `mkdir -p frontend/dist && touch frontend/dist/.ci-placeholder` を実行して `main.go` の `//go:embed all:frontend/dist` を満たす必要あり (spec-folder-watch.md §1 DoD と同じ手順)。
- [x] **CI**: GitHub Actions で 2 yaml 構成 (テスト常時 + tag 駆動リリース)。
  - 結論:
    - **`.github/workflows/ci.yml`** — `pull_request` (branches: main) + `workflow_dispatch` をトリガに、`ubuntu-latest` で走る健全性チェック (push 単独では走らない)。
      - ステップ: `actions/setup-go@v6` (`go-version: '1.26.2'`、`.go-version` を置かない方針に従って直書き) → `actions/setup-node@v6` (Node 22) → `mkdir -p frontend/dist && touch frontend/dist/.ci-placeholder` (main の `//go:embed all:frontend/dist` を満たすダミー、テストは embedded FS を読まない) → `go test ./...` (root `app_test.go` + internal 配下を一括) → `cd frontend && npm ci && npm run test && npx tsc --noEmit`。
    ...
    - **`.github/workflows/release.yml`** — `push: tags: ['v*']` のみをトリガに、`windows-latest` 1 ジョブで `wails build` から GitHub Releases までを通す。
      - ステップ: `actions/setup-go@v6` (`GO_VERSION: 1.26.2`) → `actions/setup-node@v6` (`NODE_VERSION: 22`) → `go install github.com/wailsapp/wails/v2/cmd/wails@v2.12.0` (`WAILS_CLI_VERSION` env でピン) → tag から `v` を剥がして `${VERSION}` 抽出 → `wails.json` の `info.productVersion` を `sed` で一時上書き (コミットしない) → `wails build -platform windows/amd64 -nsis -ldflags "-X main.Version=${TAG}"` → `softprops/action-gh-release@v3` で portable EXE と NSIS インストーラを Release に添付。`fail_on_unmatched_files: true` で誤動作防止。
