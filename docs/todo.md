...
- [ ] **ビューアタブ最大数の設定化 (#148)**: `settings.maxViewers` (既定 8 / 範囲 1..32、additive・version bump なし) でタブ追加上限を可変に。上限強制は**追加時 gate のみ** — state.json 復元の truncate は設定値でなくハードキャップ 32 (`state.maxViewersHard`) に緩和し、上限を下げても open 中 viewer / 復元 session を壊さない (2026-07-20 ユーザー合意)。Ctrl+Shift+2..9 は先頭 8 個据え置き。live 反映 (`maxImagePixelsMP` と同じ settings live 値 + 定数 fallback パターン)。 → [spec-viewer-max-count.md](spec-viewer-max-count.md)
