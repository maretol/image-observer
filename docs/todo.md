...
- [x] **レビュー往復の再発防止 (#110)**: PR #109 (6 round) / PR #75 (16 round) の post-mortem を反映。docs A+D のみ着手 — AGENTS.md H-8 (着手前マトリクス + 5 列同期モデルテンプレ) / 新規 I-1 (3 round 超で立ち止まる) / CLAUDE.md「非同期処理の着手前ルール」。test 基盤導入 (B = testing-library 追加、CLAUDE.md ライブラリ合意要) と責務リファクタ (C = onSave context API 化、B 依存) は別 issue に切り出してフォローアップ。 → 振り返り issue #110
