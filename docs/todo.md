...
- [ ] **画像の並び替え (#144)**: 一覧タブの表示順を 2 層で制御 — 手動順の正本 = `_classification.json` の entries 配列順 (現状の実体を公式化、順序専用の永続データは導入しない) + 表示派生ソート (`manual` 既定 / name / mtime × 昇降の 5 モード、`ListTabState.Sort` に v6 無バンプ additive 永続化、#116 と同じ整理)。mtime は `LoadResult.FileTimes` (Load 時 stat) で供給し sidecar には書かない。**Phase 1 = ソートモード / Phase 2 = 明示的な並べ替えモード + 手動 DnD** (2026-07-18 レビューで「常時 DnD でなくモード制」に確定 — モード中はプレビュー / 選択 / ビューアタブ移動等を禁止し並べ替え専念。入口は manual + フィルタ非適用、同一グループ内のみ、既存 `SaveClassification` + mtime 楽観ロックで保存)。H-8 同期モデル表を spec §8 に着手前記載。Phase 1 は実装済み (feat/144-image-sort)、Phase 2 が残るためチェックは Phase 2 完了時。 → [spec-image-sort.md](spec-image-sort.md)
