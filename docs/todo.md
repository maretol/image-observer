...
- [x] **コードサイニング**: 公的 CA / EV 署名は v1 対象外 (一般配布は未署名 = SmartScreen 警告は受容)。
  - [ ] **自己署名 (#61)**: 開発者自身の PC で警告なく起動するため、CI (`release.yml`) で portable EXE を自己署名証明書で署名する。証明書 (.pfx) は GitHub Secrets 経由、未設定時は署名スキップ。自己署名のため「証明書を信頼した PC でのみ」警告が消える (第三者は未署名同様)。 → [spec-self-signing.md](spec-self-signing.md)
