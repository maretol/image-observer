...
describe("pickGridNeighbor", () => {
  ...
  it("moves left/right in reading order", () => {
    expect(pickGridNeighbor(g, 5, "right")).toBe(6);
    expect(pickGridNeighbor(g, 5, "left")).toBe(4);
  });

  it("moves up/down to the same column in the adjacent row", () => {
    expect(pickGridNeighbor(g, 5, "up")).toBe(1);
    expect(pickGridNeighbor(g, 5, "down")).toBe(9);
  });
  ...
  it("down from a full row lands on the closest column of a partial row", () => {
    ...
    expect(pickGridNeighbor(g, 6, "down")).toBe(9);
  });

  it("returns null at the edges", () => {
    expect(pickGridNeighbor(g, 0, "left")).toBeNull();
    expect(pickGridNeighbor(g, 9, "right")).toBeNull();
    expect(pickGridNeighbor(g, 1, "up")).toBeNull(); // top row
    expect(pickGridNeighbor(g, 9, "down")).toBeNull(); // bottom row
  });
  ...
  it("crosses rows when moving right past the end of a row", () => {
    ...
    expect(pickGridNeighbor(g, 3, "right")).toBe(4);
  });

  it("up/down are null in a single-row grid", () => {
    const oneRow = grid(3, 4);
    expect(pickGridNeighbor(oneRow, 1, "up")).toBeNull();
    expect(pickGridNeighbor(oneRow, 1, "down")).toBeNull();
  });
  ...
  it("picks the nearest row, not a farther aligned one", () => {
    ...
    expect(pickGridNeighbor(g, 8, "up")).toBe(4);
  });

  it("returns null for an out-of-range index", () => {
    expect(pickGridNeighbor(g, -1, "right")).toBeNull();
    expect(pickGridNeighbor(g, 99, "up")).toBeNull();
  });
});
