...
- **`docs/spec-*.md` が実装最終形と一致**しているか。レビュー対応で実装が iterate
  ...
  - 過去事例 (PR #75): spec-folder-watch.md が **6 ラウンド連続**で「現状実装と
    ...
    返さなかったため。
- **チェックボックス / DoD / ステータス表記が「実装状態」と一致**しているか。本 PR で
  完了させた項目を `- [ ]` のまま残すと、レビュアー (特に Copilot) に「実装済みなのに
  未完了表記」とほぼ確実に指摘される (A-1 拡張「残作業 vs 実装履歴」ドリフトの一種)。
  **push 直前に、本 PR で変更した doc 内の未チェック項目を機械的に洗い出す**:
  ```bash
  git diff main...HEAD --name-only -- '*.md' \
    | xargs grep -nE '^\s*- \[ \]' 2>/dev/null
  ```
  ヒットした各項目が「本当に未着手」か「本 PR で完了したのに未チェック」かを判定し、後者は
  `[x]` + PR/issue 参照に更新する (Secrets 登録など **手作業の検証だけが残る** 場合は、
  その旨を parenthetical で明記したうえで `[x]` にしてよい。§I の PR #103 / #124 が前例)。
  - 過去事例 (PR #124): CI 署名ステップを実装した同じ PR 内で、spec-self-signing.md の
    DoD と todo.md §I 自己署名項目が `- [ ]` のまま残った。spec 側は Copilot に指摘され、
    todo.md 側は次ラウンド直前のセルフレビューで捕捉した (= 本チェックがあれば初回で防げた)。
